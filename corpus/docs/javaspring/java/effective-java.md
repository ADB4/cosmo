...
Visit us on the Web: [informit.com/aw](http://www.informit.com/aw)
...
| Foreword Preface | Acknowledgments                                                                      | xi xiii xvii                |
|------------------|--------------------------------------------------------------------------------------|-----------------------------|
| 1                | Introduction                                                                         | 1                           |
...
| Item 19: Item 20: Item 21: Item 22: Item 23: Item 24: Item 25: | Prefer interfaces to abstract classes                                              | Design and document for inheritance or else prohibit it 93 . . . . . . . . . . . . . . . . . 99 Design interfaces for posterity . . . . . . . . . . . . . . . . . . . . 104 Use interfaces only to define types. . . . . . . . . . . . . . . . . 107 Prefer class hierarchies to tagged classes . . . . . . . . . . . . 109 Favor static member classes over nonstatic . . . . . . . . . . 112 Limit source files to a single top-level class . . . . . . . . . 115 |
|----------------------------------------------------------------|------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 5 Generics                                                     |                                                                                    | 117                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
<span id="page-11-0"></span>**I**F a colleague were to say to you, "Spouse of me this night today manufactures the unusual meal in a home. You will join?" three things would likely cross your mind: third, that you had been invited to dinner; second, that English was not your colleague's first language; and first, a good deal of puzzlement.
...
> Guy L. Steele Jr. *Burlington, Massachusetts April 2001*

### <span id="page-13-0"></span>**Preface to the Third Edition**

**I**N 1997, when Java was new, James Gosling (the father of Java), described it as a "blue collar language" that was "pretty simple" [Gosling97]. At about the same time, Bjarne Stroustrup (the father of C++) described C++ as a "multi-paradigm language" that "deliberately differs from languages designed to support a single way of writing programs" [Stroustrup95]. Stroustrup warned:
...
### <span id="page-17-0"></span>**Acknowledgments for the Third Edition**
...
<span id="page-21-0"></span>**T**HIS book is designed to help you make effective use of the Java programming language and its fundamental libraries: java.lang, java.util, and java.io, and subpackages such as java.util.concurrent and java.util.function. Other libraries are discussed from time to time.
...
<span id="page-22-0"></span>Most items are illustrated with program examples. A key feature of this book is that it contains code examples illustrating many design patterns and idioms. Where appropriate, they are cross-referenced to the standard reference work in this area [Gamma95].
...
<span id="page-23-0"></span>When discussing features of the Java programming language and its libraries, it is sometimes necessary to refer to specific releases. For convenience, this book uses nicknames in preference to official release names. This table shows the mapping between release names and nicknames:
...
<span id="page-24-0"></span>simply states that a class *implements* an interface or that one interface *extends* another. To describe the access level that applies when none is specified, this book uses the traditional *package-private* instead of the technically correct *package access* [JLS, 6.6.1].
...
## <span id="page-25-0"></span>[Creating and Destroying Objects](#page-7-0)
...
### **[Item 1: Consider static factory methods instead of constructors](#page-7-0)**
...
The existence of these two implementation classes is invisible to clients. If RegularEnumSet ceased to offer performance advantages for small enum types, it could be eliminated from a future release with no ill effects. Similarly, a future release could add a third or fourth implementation of EnumSet if it proved beneficial <span id="page-28-0"></span>for performance. Clients neither know nor care about the class of the object they get back from the factory; they care only that it is some subclass of EnumSet.
...
### <span id="page-30-0"></span>**[Item 2: Consider a builder when faced with many constructor](#page-7-0) parameters**
...
**// Telescoping constructor pattern - does not scale well!** public class NutritionFacts { private final int servingSize; // (mL) required private final int servings; // (per container) required private final int calories; // (per serving) optional private final int fat; // (g/serving) optional private final int sodium; // (mg/serving) optional private final int carbohydrate; // (g/serving) optional public NutritionFacts(int servingSize, int servings) { this(servingSize, servings, 0); } public NutritionFacts(int servingSize, int servings, int calories) { this(servingSize, servings, calories, 0); } public NutritionFacts(int servingSize, int servings, int calories, int fat) { this(servingSize, servings, calories, fat, 0); } public NutritionFacts(int servingSize, int servings, int calories, int fat, int sodium) { this(servingSize, servings, calories, fat, sodium, 0); }

<span id="page-31-0"></span>public NutritionFacts(int servingSize, int servings, int calories, int fat, int sodium, int carbohydrate) { this.servingSize = servingSize; this.servings = servings; this.calories = calories; this.fat = fat; this.sodium = sodium; this.carbohydrate = carbohydrate; } }
...
**// JavaBeans Pattern - allows inconsistency, mandates mutability** public class NutritionFacts { // Parameters initialized to default values (if any) private int servingSize = -1; // Required; no default value private int servings = -1; // Required; no default value private int calories = 0; private int fat = 0; private int sodium = 0; private int carbohydrate = 0; public NutritionFacts() { }

<span id="page-32-0"></span>// Setters public void setServingSize(int val) { servingSize = val; } public void setServings(int val) { servings = val; } public void setCalories(int val) { calories = val; } public void setFat(int val) { fat = val; } public void setSodium(int val) { sodium = val; } public void setCarbohydrate(int val) { carbohydrate = val; } }
...
<span id="page-34-0"></span>The NutritionFacts class is immutable, and all parameter default values are in one place. The builder's setter methods return the builder itself so that invocations can be chained, resulting in a *fluent API*. Here's how the client code looks:
...
**// Builder pattern for class hierarchies** public abstract class Pizza { public enum Topping { HAM, MUSHROOM, ONION, PEPPER, SAUSAGE } final Set<Topping> toppings; abstract static class **Builder<T extends Builder<T>>** { EnumSet<Topping> toppings = EnumSet.noneOf(Topping.class); public T addTopping(Topping topping) { toppings.add(Objects.requireNonNull(topping)); **return self();** } abstract Pizza build(); **// Subclasses must override this method to return "this" protected abstract T self();** } Pizza(Builder<?> builder) { toppings = builder.toppings.clone(); // See Item 50 } }
...
#### <span id="page-37-0"></span>**[Item 3: Enforce the singleton property with a private](#page-7-0) constructor or an enum type**
...
<span id="page-38-0"></span>The main advantage of the public field approach is that the API makes it clear that the class is a singleton: the public static field is final, so it will always contain the same object reference. The second advantage is that it's simpler.
...
### <span id="page-39-0"></span>**[Item 4: Enforce noninstantiability with a private constructor](#page-7-0)**
...
#### <span id="page-40-0"></span>**[Item 5: Prefer dependency injection to hardwiring resources](#page-7-0)**
...
<span id="page-41-0"></span>**// Dependency injection provides flexibility and testability** public class SpellChecker { private final Lexicon dictionary; **public SpellChecker(Lexicon dictionary)** { this.dictionary = Objects.requireNonNull(dictionary); } public boolean isValid(String word) { ... } public List<String> suggestions(String typo) { ... } }
...
#### <span id="page-42-0"></span>**[Item 6: Avoid creating unnecessary objects](#page-7-0)**
...
**// Hideously slow! Can you spot the object creation?** private static long sum() { Long sum = 0L; for (long i = 0; i <= Integer.MAX\_VALUE; i++) sum += i; return sum; }
...
Conversely, avoiding object creation by maintaining your own *object pool* is a bad idea unless the objects in the pool are extremely heavyweight. The classic <span id="page-45-0"></span>example of an object that *does* justify an object pool is a database connection. The cost of establishing the connection is sufficiently high that it makes sense to reuse these objects. Generally speaking, however, maintaining your own object pools clutters your code, increases memory footprint, and harms performance. Modern JVM implementations have highly optimized garbage collectors that easily outperform such object pools on lightweight objects.
...
#### <span id="page-46-0"></span>**[Item 7: Eliminate obsolete object references](#page-7-0)**
...
**// Can you spot the "memory leak"?** public class Stack { private Object[] elements; private int size = 0; private static final int DEFAULT\_INITIAL\_CAPACITY = 16; public Stack() { elements = new Object[DEFAULT\_INITIAL\_CAPACITY]; } public void push(Object e) { ensureCapacity(); elements[size++] = e; } public Object pop() { if (size == 0) throw new EmptyStackException(); return elements[--size]; } /\*\* \* Ensure space for at least one more element, roughly \* doubling the capacity each time the array needs to grow. \*/ private void ensureCapacity() { if (elements.length == size) elements = Arrays.copyOf(elements, 2 \* size + 1); } }

There's nothing obviously wrong with this program (but see Item 29 for a generic version). You could test it exhaustively, and it would pass every test with flying colors, but there's a problem lurking. Loosely speaking, the program has a "memory leak," which can silently manifest itself as reduced performance due to <span id="page-47-0"></span>increased garbage collector activity or increased memory footprint. In extreme cases, such memory leaks can cause disk paging and even program failure with an OutOfMemoryError, but such failures are relatively rare.
...
### <span id="page-49-0"></span>**[Item 8: Avoid finalizers and cleaners](#page-7-0)**
...
The static nested State class holds the resources that are required by the cleaner to clean the room. In this case, it is simply the numJunkPiles field, which represents the amount of mess in the room. More realistically, it might be a final long that contains a pointer to a native peer. State implements Runnable, and its run method is called at most once, by the Cleanable that we get when we register our State instance with our cleaner in the Room constructor. The call to the run method will be triggered by one of two things: Usually it is triggered by a call to <span id="page-53-0"></span>Room's close method calling Cleanable's clean method. If the client fails to call the close method by the time a Room instance is eligible for garbage collection, the cleaner will (hopefully) call State's run method.
...
#### <span id="page-54-0"></span>**Item 9: Prefer try[-with-resources to](#page-7-0) try-finally**
...
**// try-with-resources on multiple resources - short and sweet** static void copy(String src, String dst) throws IOException { try (InputStream in = new FileInputStream(src); OutputStream out = new FileOutputStream(dst)) { byte[] buf = new byte[BUFFER\_SIZE]; int n; while ((n = in.read(buf)) >= 0) out.write(buf, 0, n); } }

Not only are the try-with-resources versions shorter and more readable than the originals, but they provide far better diagnostics. Consider the firstLineOfFile <span id="page-56-0"></span>method. If exceptions are thrown by both the readLine call and the (invisible) close, the latter exception is *suppressed* in favor of the former. In fact, multiple exceptions may be suppressed in order to preserve the exception that you actually want to see. These suppressed exceptions are not merely discarded; they are printed in the stack trace with a notation saying that they were suppressed. You can also access them programmatically with the getSuppressed method, which was added to Throwable in Java 7.
...
## <span id="page-57-0"></span>[Methods Common to All Objects](#page-7-0)
...
### **[Item 10: Obey the general contract when overriding](#page-7-0) equals**
...
- <span id="page-58-0"></span>• **A superclass has already overridden equals**, **and the superclass behavior is appropriate for this class.** For example, most Set implementations inherit their equals implementation from AbstractSet, List implementations from AbstractList, and Map implementations from AbstractMap.
...
<span id="page-60-0"></span>The well-intentioned equals method in this class naively attempts to interoperate with ordinary strings. Let's suppose that we have one case-insensitive string and one ordinary one:
...
<span id="page-62-0"></span>The problem with this method is that you might get different results when comparing a point to a color point and vice versa. The former comparison ignores color, while the latter comparison always returns false because the type of the argument is incorrect. To make this concrete, let's create one point and one color point:
...
<span id="page-63-0"></span>You may hear it said that you can extend an instantiable class and add a value component while preserving the equals contract by using a getClass test in place of the instanceof test in the equals method:

**// Broken - violates Liskov substitution principle (page 43)** @Override public boolean equals(Object o) { **if (o == null || o.getClass() != getClass()) return false;** Point p = (Point) o; return p.x == x && p.y == y; }
...
**// Initialize unitCircle to contain all Points on the unit circle** private static final Set<Point> unitCircle = Set.of( new Point( 1, 0), new Point( 0, 1), new Point(-1, 0), new Point( 0, -1)); public static boolean onUnitCircle(Point p) { return unitCircle.contains(p); }
...
The *Liskov substitution principle* says that any important property of a type should also hold for all its subtypes so that any method written for the type should work equally well on its subtypes [Liskov87]. This is the formal statement of our <span id="page-64-0"></span>earlier claim that a subclass of Point (such as CounterPoint) is still a Point and must act as one. But suppose we pass a CounterPoint to the onUnitCircle method. If the Point class uses a getClass-based equals method, the onUnitCircle method will return false regardless of the CounterPoint instance's *x* and *y* coordinates. This is so because most collections, including the HashSet used by the onUnitCircle method, use the equals method to test for containment, and no CounterPoint instance is equal to any Point. If, however, you use a proper instanceof-based equals method on Point, the same onUnitCircle method works fine when presented with a CounterPoint instance.
...
- <span id="page-69-0"></span>• **Don't substitute another type for Object in the equals declaration.** It is not uncommon for a programmer to write an equals method that looks like this and then spend hours puzzling over why it doesn't work properly:
...
#### <span id="page-70-0"></span>**[Item 11: Always override](#page-7-0) hashCode when you override equals**
...
<span id="page-71-0"></span>Fixing this problem is as simple as writing a proper hashCode method for PhoneNumber. So what should a hashCode method look like? It's trivial to write a bad one. This one, for example, is always legal but should never be used:
...
**Do not be tempted to exclude significant fields from the hash code computation to improve performance.** While the resulting hash function may run faster, its poor quality may degrade hash tables' performance to the point where they become unusable. In particular, the hash function may be confronted with a <span id="page-74-0"></span>large collection of instances that differ mainly in regions you've chosen to ignore. If this happens, the hash function will map all these instances to a few hash codes, and programs that should run in linear time will instead run in quadratic time.
...
### <span id="page-75-0"></span>**[Item 12: Always override](#page-7-0) toString**
...
<span id="page-77-0"></span>If you decide not to specify a format, the documentation comment should read something like this:
...
#### <span id="page-78-0"></span>**[Item 13: Override](#page-7-0) clone judiciously**
...
<span id="page-79-0"></span>By convention, the object returned by this method should be obtained by calling super.clone. If a class and all of its superclasses (except Object) obey this convention, it will be the case that
...
In order for this method to work, the class declaration for PhoneNumber would have to be modified to indicate that it implements Cloneable. Though Object's clone method returns Object, this clone method returns PhoneNumber. It is legal <span id="page-80-0"></span>and desirable to do this because Java supports *covariant return types*. In other words, an overriding method's return type can be a subclass of the overridden method's return type. This eliminates the need for casting in the client. We must cast the result of super.clone from Object to PhoneNumber before returning it, but the cast is guaranteed to succeed.
...
public class Stack { private Object[] elements; private int size = 0; private static final int DEFAULT\_INITIAL\_CAPACITY = 16; public Stack() { this.elements = new Object[DEFAULT\_INITIAL\_CAPACITY]; } public void push(Object e) { ensureCapacity(); elements[size++] = e; } public Object pop() { if (size == 0) throw new EmptyStackException(); Object result = elements[--size]; elements[size] = null; // Eliminate obsolete reference return result; } // Ensure space for at least one more element. private void ensureCapacity() { if (elements.length == size) elements = Arrays.copyOf(elements, 2 \* size + 1); } }

Suppose you want to make this class cloneable. If the clone method merely returns super.clone(), the resulting Stack instance will have the correct value in <span id="page-81-0"></span>its size field, but its elements field will refer to the same array as the original Stack instance. Modifying the original will destroy the invariants in the clone and vice versa. You will quickly find that your program produces nonsensical results or throws a NullPointerException.
...
**// Recursive clone method for class with complex mutable state** public class HashTable implements Cloneable { private Entry[] buckets = ...; private static class Entry { final Object key; Object value; Entry next; Entry(Object key, Object value, Entry next) { this.key = key; this.value = value; this.next = next; }
...
#### <span id="page-86-0"></span>**[Item 14: Consider implementing](#page-7-0) Comparable**
...
<span id="page-87-0"></span>In the following description, the notation sgn(*expression*) designates the mathematical *signum* function, which is defined to return -1, 0, or 1, according to whether the value of *expression* is negative, zero, or positive.
...
<span id="page-89-0"></span>In a compareTo method, fields are compared for order rather than equality. To compare object reference fields, invoke the compareTo method recursively. If a field does not implement Comparable or you need a nonstandard ordering, use a Comparator instead. You can write your own comparator or use an existing one, as in this compareTo method for CaseInsensitiveString in Item 10:

**// Single-field Comparable with object reference field** public final class CaseInsensitiveString implements Comparable<CaseInsensitiveString> { public int compareTo(CaseInsensitiveString cis) { return String.CASE\_INSENSITIVE\_ORDER.compare(s, cis.s); } ... // Remainder omitted }
...
If a class has multiple significant fields, the order in which you compare them is critical. Start with the most significant field and work your way down. If a comparison results in anything other than zero (which represents equality), you're done; just return the result. If the most significant field is equal, compare the nextmost-significant field, and so on, until you find an unequal field or compare the least significant field. Here is a compareTo method for the PhoneNumber class in Item 11 demonstrating this technique:
...
**// BROKEN difference-based comparator - violates transitivity!** static Comparator<Object> hashCodeOrder = new Comparator<>() { public int compare(Object o1, Object o2) { return o1.hashCode() - o2.hashCode(); } };
...
private static final Thing[] PRIVATE\_VALUES = { ... }; public static final List<Thing> VALUES = Collections.unmodifiableList(Arrays.asList(PRIVATE\_VALUES));

Alternatively, you can make the array private and add a public method that returns a copy of a private array:

private static final Thing[] PRIVATE\_VALUES = { ... }; public static final Thing[] values() { return PRIVATE\_VALUES.clone(); }
...
#### <span id="page-98-0"></span>**[Item 16: In public classes, use accessor methods, not public fields](#page-7-0)**
...
However, **if a class is package-private or is a private nested class, there is nothing inherently wrong with exposing its data fields**—assuming they do an <span id="page-99-0"></span>adequate job of describing the abstraction provided by the class. This approach generates less visual clutter than the accessor-method approach, both in the class definition and in the client code that uses it. While the client code is tied to the class's internal representation, this code is confined to the package containing the class. If a change in representation becomes desirable, you can make the change without touching any code outside the package. In the case of a private nested class, the scope of the change is further restricted to the enclosing class.
...
**// Public class with exposed immutable fields - questionable** public final class Time { private static final int HOURS\_PER\_DAY = 24; private static final int MINUTES\_PER\_HOUR = 60; **public final int hour; public final int minute;** public Time(int hour, int minute) { if (hour < 0 || hour >= HOURS\_PER\_DAY) throw new IllegalArgumentException("Hour: " + hour); if (minute < 0 || minute >= MINUTES\_PER\_HOUR) throw new IllegalArgumentException("Min: " + minute); this.hour = hour; this.minute = minute; } ... // Remainder omitted }

In summary, public classes should never expose mutable fields. It is less harmful, though still questionable, for public classes to expose immutable fields. It is, however, sometimes desirable for package-private or private nested classes to expose fields, whether mutable or immutable.

### <span id="page-100-0"></span>**[Item 17: Minimize mutability](#page-7-0)**
...
This approach can be taken one step further. An immutable class can provide static factories (Item 1) that cache frequently requested instances to avoid creating new instances when existing ones would do. All the boxed primitive classes and BigInteger do this. Using such static factories causes clients to share instances instead of creating new ones, reducing memory footprint and garbage collection <span id="page-103-0"></span>costs. Opting for static factories in place of public constructors when designing a new class gives you the flexibility to add caching later, without modifying clients.
...
One caveat should be added concerning serializability. If you choose to have your immutable class implement Serializable and it contains one or more fields that refer to mutable objects, you must provide an explicit readObject or readResolve method, or use the ObjectOutputStream.writeUnshared and <span id="page-106-0"></span>ObjectInputStream.readUnshared methods, even if the default serialized form is acceptable. Otherwise an attacker could create a mutable instance of your class. This topic is covered in detail in Item 88.
...
### <span id="page-107-0"></span>**[Item 18: Favor composition over inheritance](#page-7-0)**
...
**// Broken - Inappropriate use of inheritance!** public class InstrumentedHashSet<E> extends HashSet<E> { // The number of attempted element insertions private int addCount = 0; public InstrumentedHashSet() { } public InstrumentedHashSet(int initCap, float loadFactor) { super(initCap, loadFactor); }

@Override public boolean add(E e) { addCount++; return super.add(e); } @Override public boolean addAll(Collection<? extends E> c) { addCount += c.size(); return super.addAll(c); } public int getAddCount() { return addCount; } }
...
It would be slightly better to override the addAll method to iterate over the specified collection, calling the add method once for each element. This would guarantee the correct result whether or not HashSet's addAll method were implemented atop its add method because HashSet's addAll implementation would no longer be invoked. This technique, however, does not solve all our problems. It amounts to reimplementing superclass methods that may or may not <span id="page-109-0"></span>result in self-use, which is difficult, time-consuming, error-prone, and may reduce performance. Additionally, it isn't always possible because some methods cannot be implemented without access to private fields inaccessible to the subclass.
...
<span id="page-111-0"></span>The design of the InstrumentedSet class is enabled by the existence of the Set interface, which captures the functionality of the HashSet class. Besides being robust, this design is extremely flexible. The InstrumentedSet class implements the Set interface and has a single constructor whose argument is also of type Set. In essence, the class transforms one Set into another, adding the instrumentation functionality. Unlike the inheritance-based approach, which works only for a single concrete class and requires a separate constructor for each supported constructor in the superclass, the wrapper class can be used to instrument any Set implementation and will work in conjunction with any preexisting constructor:

Set<Instant> times = new InstrumentedSet<>(new TreeSet<>(cmp)); Set<E> s = new InstrumentedSet<>(new HashSet<>(INIT\_CAPACITY));
...
### <span id="page-113-0"></span>**[Item 19: Design and document for inheritance or else prohibit it](#page-8-0)**
...
<span id="page-116-0"></span>Here's a subclass that overrides the overrideMe method, which is erroneously invoked by Super's sole constructor:
...
If you do decide to implement either Cloneable or Serializable in a class that is designed for inheritance, you should be aware that because the clone and readObject methods behave a lot like constructors, a similar restriction applies: **neither clone nor readObject may invoke an overridable method, directly or indirectly.** In the case of readObject, the overriding method will run before the <span id="page-117-0"></span>subclass's state has been deserialized. In the case of clone, the overriding method will run before the subclass's clone method has a chance to fix the clone's state. In either case, a program failure is likely to follow. In the case of clone, the failure can damage the original object as well as the clone. This can happen, for example, if the overriding method assumes it is modifying the clone's copy of the object's deep structure, but the copy hasn't been made yet.
...
### <span id="page-119-0"></span>**[Item 20: Prefer interfaces to abstract classes](#page-8-0)**
...
<span id="page-120-0"></span>public interface Songwriter { Song compose(int chartPosition); }
...
You can, however, combine the advantages of interfaces and abstract classes by providing an abstract *skeletal implementation class* to go with an interface. The interface defines the type, perhaps providing some default methods, while the skeletal implementation class implements the remaining non-primitive interface <span id="page-121-0"></span>methods atop the primitive interface methods. Extending a skeletal implementation takes most of the work out of implementing an interface. This is the *Template Method* pattern [Gamma95].
...
#### <span id="page-124-0"></span>**[Item 21: Design interfaces for posterity](#page-8-0)**
...
<span id="page-126-0"></span>It is also worth noting that default methods were not designed to support removing methods from interfaces or changing the signatures of existing methods. Neither of these interface changes is possible without breaking existing clients.
...
#### <span id="page-127-0"></span>**[Item 22: Use interfaces only to define types](#page-8-0)**
 ...
 **// Constant interface antipattern - do not use!** public interface PhysicalConstants { // Avogadro's number (1/mol) static final double AVOGADROS\_NUMBER = 6.022\_140\_857e23; // Boltzmann constant (J/K) static final double BOLTZMANN\_CONSTANT = 1.380\_648\_52e-23; // Mass of the electron (kg) static final double ELECTRON\_MASS = 9.109\_383\_56e-31; }
...
If you want to export constants, there are several reasonable choices. If the constants are strongly tied to an existing class or interface, you should add them to the class or interface. For example, all of the boxed numerical primitive classes, such as Integer and Double, export MIN\_VALUE and MAX\_VALUE constants. If the constants are best viewed as members of an enumerated type, you should export <span id="page-128-0"></span>them with an *enum type* (Item 34). Otherwise, you should export the constants with a noninstantiable *utility class* (Item 4). Here is a utility class version of the PhysicalConstants example shown earlier:

**// Constant utility class** package com.effectivejava.science; public class PhysicalConstants { private PhysicalConstants() { } // Prevents instantiation public static final double AVOGADROS\_NUMBER = 6.022\_140\_857e23; public static final double BOLTZMANN\_CONST = 1.380\_648\_52e-23; public static final double ELECTRON\_MASS = 9.109\_383\_56e-31; }
...
**// Use of static import to avoid qualifying constants** import static com.effectivejava.science.PhysicalConstants.\*; public class Test { double atoms(double mols) { return AVOGADROS\_NUMBER \* mols; } ... // Many more uses of PhysicalConstants justify static import }

In summary, interfaces should be used only to define types. They should not be used merely to export constants.

#### <span id="page-129-0"></span>**[Item 23: Prefer class hierarchies to tagged classes](#page-8-0)**
...
**// Class hierarchy replacement for a tagged class** abstract class Figure { abstract double area(); } class Circle extends Figure { final double radius; Circle(double radius) { this.radius = radius; } @Override double area() { return Math.PI \* (radius \* radius); } }

<span id="page-131-0"></span>class Rectangle extends Figure { final double length; final double width; Rectangle(double length, double width) { this.length = length; this.width = width; } @Override double area() { return length \* width; } }
...
#### <span id="page-132-0"></span>**[Item 24: Favor static member classes over nonstatic](#page-8-0)**
...
<span id="page-133-0"></span>One common use of a nonstatic member class is to define an *Adapter* [Gamma95] that allows an instance of the outer class to be viewed as an instance of some unrelated class. For example, implementations of the Map interface typically use nonstatic member classes to implement their *collection views*, which are returned by Map's keySet, entrySet, and values methods. Similarly, implementations of the collection interfaces, such as Set and List, typically use nonstatic member classes to implement their iterators:
...
It is doubly important to choose correctly between a static and a nonstatic member class if the class in question is a public or protected member of an <span id="page-134-0"></span>exported class. In this case, the member class is an exported API element and cannot be changed from a nonstatic to a static member class in a subsequent release without violating backward compatibility.
...
### <span id="page-135-0"></span>**[Item 25: Limit source files to a single top-level class](#page-8-0)**
...
<span id="page-140-0"></span>This program compiles, but because it uses the raw type List, you get a warning:
...
**// Legitimate use of raw type - instanceof operator** if (o instanceof **Set**) { // Raw type **Set<?>** s = **(Set<?>)** o; // Wildcard type ... }

<span id="page-142-0"></span>Note that once you've determined that o is a Set, you must cast it to the wildcard type Set<?>, not the raw type Set. This is a checked cast, so it will not cause a compiler warning.
...
### <span id="page-143-0"></span>**[Item 27: Eliminate unchecked warnings](#page-8-0)**
...
<span id="page-145-0"></span>The resulting method compiles cleanly and minimizes the scope in which unchecked warnings are suppressed.
...
#### <span id="page-146-0"></span>**[Item 28: Prefer lists to arrays](#page-8-0)**
...
<span id="page-147-0"></span>To make this more concrete, consider the following code fragment:
...
**// A first cut at making Chooser generic - won't compile** public class Chooser**<T>** { private final **T**[] choiceArray; public Chooser(Collection**<T>** choices) { choiceArray = choices.toArray(); } // choose method unchanged }
...
<span id="page-149-0"></span>No big deal, you say, I'll cast the Object array to a T array:
...
**// List-based Chooser - typesafe** public class Chooser<T> { private final **List<T>** choiceList; public Chooser(Collection<T> choices) { choiceList = **new ArrayList<>(choices)**; } public T choose() { Random rnd = ThreadLocalRandom.current(); return **choiceList.get(rnd.nextInt(choiceList.size()))**; } }
...
#### <span id="page-150-0"></span>**[Item 29: Favor generic types](#page-8-0)**
...
**// Initial attempt to generify Stack - won't compile!** public class Stack**<E>** { private **E**[] elements; private int size = 0; private static final int DEFAULT\_INITIAL\_CAPACITY = 16; public Stack() { elements = new **E**[DEFAULT\_INITIAL\_CAPACITY]; } public void push(**E** e) { ensureCapacity(); elements[size++] = e; } public **E** pop() { if (size == 0) throw new EmptyStackException(); **E** result = elements[--size]; elements[size] = null; // Eliminate obsolete reference return result; } ... // no changes in isEmpty or ensureCapacity }
...
Stack.java:8: generic array creation elements = **new E[DEFAULT\_INITIAL\_CAPACITY]**; ^
...
Stack.java:8: warning: [unchecked] unchecked cast found: Object[], required: E[] elements = **(E[]) new Object[DEFAULT\_INITIAL\_CAPACITY]**; ^
...
**// The elements array will contain only E instances from push(E). // This is sufficient to ensure type safety, but the runtime // type of the array won't be E[]; it will always be Object[]! @SuppressWarnings("unchecked")**  public Stack() { elements = (E[]) new Object[DEFAULT\_INITIAL\_CAPACITY]; }
...
### <span id="page-155-0"></span>**[Item 30: Favor generic methods](#page-8-0)**
...
At least for simple generic methods, that's all there is to it. This method compiles without generating any warnings and provides type safety as well as ease of <span id="page-156-0"></span>use. Here's a simple program to exercise the method. This program contains no casts and compiles without errors or warnings:
...
**// Generic singleton factory pattern** private static UnaryOperator<Object> IDENTITY\_FN = (t) -> t; **@SuppressWarnings("unchecked")** public static <T> UnaryOperator<T> identityFunction() { return **(UnaryOperator<T>)** IDENTITY\_FN; }

The cast of IDENTITY\_FN to (UnaryFunction<T>) generates an unchecked cast warning, as UnaryOperator<Object> is not a UnaryOperator<T> for every T. But the identity function is special: it returns its argument unmodified, so we know that it is typesafe to use it as a UnaryFunction<T>, whatever the value of T. <span id="page-157-0"></span>Therefore, we can confidently suppress the unchecked cast warning generated by this cast. Once we've done this, the code compiles without error or warning.
...
**// Using a recursive type bound to express mutual comparability** public static **<E extends Comparable<E>>** E max(Collection<E> c); <span id="page-158-0"></span>The type bound <E extends Comparable<E>> may be read as "any type E that can be compared to itself," which corresponds more or less precisely to the notion of mutual comparability.
...
**// Returns max value in a collection - uses recursive type bound** public static <E extends Comparable<E>> E max(Collection<E> c) { if (c.isEmpty()) throw new IllegalArgumentException("Empty collection"); E result = null; for (E e : c) if (result == null || e.compareTo(result) > 0) result = Objects.requireNonNull(e); return result; }
...
#### <span id="page-159-0"></span>**[Item 31: Use bounded wildcards to increase API flexibility](#page-8-0)**
...
If you try to compile this client code against the version of popAll shown earlier, you'll get an error very similar to the one that we got with our first version of pushAll: Collection<Object> is not a subtype of Collection<Number>. Once again, wildcard types provide a way out. The type of the input parameter to <span id="page-161-0"></span>popAll should not be "collection of E" but "collection of some supertype of E" (where supertype is defined such that E is a supertype of itself [JLS, 4.10]). Again, there is a wildcard type that means precisely that: Collection<? super E>. Let's modify popAll to use it:
...
And would this change make any difference in practice? Yes, it would. Suppose you have a List<Integer>, and you want to pass it in to the constructor <span id="page-162-0"></span>for a Chooser<Number>. This would not compile with the original declaration, but it does once you add the bounded wildcard type to the declaration.
...
The reason that you can't apply the original method declaration to this list is that ScheduledFuture does not implement Comparable<ScheduledFuture>. Instead, it is a subinterface of Delayed, which extends Comparable<Delayed>. In other words, a ScheduledFuture instance isn't merely comparable to other <span id="page-164-0"></span>ScheduledFuture instances; it is comparable to any Delayed instance, and that's enough to cause the original declaration to reject it. More generally, the wildcard is required to support types that do not implement Comparable (or Comparator) directly but extend a type that does.
...
It doesn't seem right that we can't put an element back into the list that we just took it out of. The problem is that the type of list is List<?>, and you can't put any value except null into a List<?>. Fortunately, there is a way to implement this method without resorting to an unsafe cast or a raw type. The idea is to write a <span id="page-165-0"></span>private helper method to *capture* the wildcard type. The helper method must be a generic method in order to capture the type. Here's how it looks:
...
#### <span id="page-166-0"></span>**[Item 32: Combine generics and varargs judiciously](#page-8-0)**
...
<span id="page-168-0"></span>To make this concrete, consider the following generic method, which takes three arguments of type T and returns an array containing two of the arguments, chosen at random:
...
<span id="page-170-0"></span>This method can then be used in conjunction with the static factory method List.of to allow for a variable number of arguments. Note that this approach relies on the fact that the List.of declaration is annotated with @SafeVarargs:
...
### <span id="page-171-0"></span>**[Item 33: Consider typesafe heterogeneous containers](#page-8-0)**
...
**// Typesafe heterogeneous container pattern - client** public static void main(String[] args) { Favorites f = new Favorites(); f.putFavorite(String.class, "Java"); f.putFavorite(Integer.class, 0xcafebabe); f.putFavorite(Class.class, Favorites.class);

<span id="page-172-0"></span>String favoriteString = f.getFavorite(String.class); int favoriteInteger = f.getFavorite(Integer.class); Class<?> favoriteClass = f.getFavorite(Class.class); System.out.printf("%s %x %s%n", favoriteString, favoriteInteger, favoriteClass.getName()); }
...
public <T extends Annotation> T getAnnotation(Class<T> annotationType); <span id="page-175-0"></span>The argument, annotationType, is a bounded type token representing an annotation type. The method returns the element's annotation of that type, if it has one, or null, if it doesn't. In essence, an annotated element is a typesafe heterogeneous container whose keys are annotation types.
...
## [Enums and Annotations](#page-8-0)

<span id="page-177-0"></span>**J**AVA supports two special-purpose families of reference types: a kind of class called an *enum type,* and a kind of interface called an *annotation type*. This chapter discusses best practices for using these type families.

#### **[Item 34: Use enums instead of](#page-8-0) int constants**
...
**// Tasty citrus flavored applesauce!** int i = (APPLE\_FUJI - ORANGE\_TEMPLE) / APPLE\_PIPPIN;

Note that the name of each apple constant is prefixed with APPLE\_ and the name of each orange constant is prefixed with ORANGE\_. This is because Java <span id="page-178-0"></span>doesn't provide namespaces for int enum groups. Prefixes prevent name clashes when two int enum groups have identically named constants, for example between ELEMENT\_MERCURY and PLANET\_MERCURY.
...
Enums provide compile-time type safety. If you declare a parameter to be of type Apple, you are guaranteed that any non-null object reference passed to the parameter is one of the three valid Apple values. Attempts to pass values of the <span id="page-179-0"></span>wrong type will result in compile-time errors, as will attempts to assign an expression of one enum type to a variable of another, or to use the == operator to compare values of different enum types.
...
**// Enum type with data and behavior** public enum Planet { MERCURY(3.302e+23, 2.439e6), VENUS (4.869e+24, 6.052e6), EARTH (5.975e+24, 6.378e6), MARS (6.419e+23, 3.393e6), JUPITER(1.899e+27, 7.149e7), SATURN (5.685e+26, 6.027e7), URANUS (8.683e+25, 2.556e7), NEPTUNE(1.024e+26, 2.477e7); private final double mass; // In kilograms private final double radius; // In meters private final double surfaceGravity; // In m / s^2

<span id="page-180-0"></span>// Universal gravitational constant in m^3 / kg s^2 private static final double G = 6.67300E-11; // Constructor Planet(double mass, double radius) { this.mass = mass; this.radius = radius; surfaceGravity = G \* mass / (radius \* radius); } public double mass() { return mass; } public double radius() { return radius; } public double surfaceGravity() { return surfaceGravity; } public double surfaceWeight(double mass) { return mass \* surfaceGravity; // F = ma } }
...
Note that Planet, like all enums, has a static values method that returns an array of its values in the order they were declared. Note also that the toString method returns the declared name of each enum value, enabling easy printing by println and printf. If you're dissatisfied with this string representation, you can change it <span id="page-181-0"></span>by overriding the toString method. Here is the result of running our WeightTable program (which doesn't override toString) with the command line argument 185:
...
The techniques demonstrated in the Planet example are sufficient for most enum types, but sometimes you need more. There is different data associated with each Planet constant, but sometimes you need to associate fundamentally different *behavior* with each constant. For example, suppose you are writing an enum <span id="page-182-0"></span>type to represent the operations on a basic four-function calculator and you want to provide a method to perform the arithmetic operation represented by each constant. One way to achieve this is to switch on the value of the enum:
...
<span id="page-184-0"></span>Enum types have an automatically generated valueOf(String) method that translates a constant's name into the constant itself. If you override the toString method in an enum type, consider writing a fromString method to translate the custom string representation back to the corresponding enum. The following code (with the type name changed appropriately) will do the trick for any enum, so long as each constant has a unique string representation:
...
**// Enum that switches on its value to share code - questionable** enum PayrollDay { MONDAY, TUESDAY, WEDNESDAY, THURSDAY, FRIDAY, SATURDAY, SUNDAY; private static final int MINS\_PER\_SHIFT = 8 \* 60; int pay(int minutesWorked, int payRate) { int basePay = minutesWorked \* payRate; int overtimePay; **switch(this) { case SATURDAY: case SUNDAY: // Weekend overtimePay = basePay / 2; break; default: // Weekday overtimePay = minutesWorked <= MINS\_PER\_SHIFT ? 0 : (minutesWorked - MINS\_PER\_SHIFT) \* payRate / 2; }** return basePay + overtimePay; } }
...
#### <span id="page-188-0"></span>**[Item 35: Use instance fields instead of ordinals](#page-8-0)**
...
public enum Ensemble { SOLO(1), DUET(2), TRIO(3), QUARTET(4), QUINTET(5), SEXTET(6), SEPTET(7), OCTET(8), DOUBLE\_QUARTET(8), NONET(9), DECTET(10), TRIPLE\_QUARTET(12); private final int numberOfMusicians; Ensemble(int size) { this.numberOfMusicians = size; } public int numberOfMusicians() { return numberOfMusicians; } }
...
#### <span id="page-189-0"></span>**Item 36: Use EnumSet [instead of bit fields](#page-8-0)**
...
**// Bit field enumeration constants - OBSOLETE!** public class Text { public static final int STYLE\_BOLD = 1 << 0; // 1 public static final int STYLE\_ITALIC = 1 << 1; // 2 public static final int STYLE\_UNDERLINE = 1 << 2; // 4 public static final int STYLE\_STRIKETHROUGH = 1 << 3; // 8 // Parameter is bitwise OR of zero or more STYLE\_ constants public void applyStyles(int styles) { ... } }

This representation lets you use the bitwise OR operation to combine several constants into a set, known as a *bit field*:

text.applyStyles(STYLE\_BOLD | STYLE\_ITALIC);
...
#### <span id="page-191-0"></span>**Item 37: Use EnumMap [instead of ordinal indexing](#page-8-0)**
...
The problem with this code is that it chooses its own map implementation, and in practice it won't be an EnumMap, so it won't match the space and time performance of the version with the explicit EnumMap. To rectify this problem, use the three<span id="page-193-0"></span>parameter form of Collectors.groupingBy, which allows the caller to specify the map implementation using the mapFactory parameter:

**// Using a stream and an EnumMap to associate data with an enum** System.out.println(Arrays.stream(garden) .collect(groupingBy(p -> p.lifeCycle, **() -> new EnumMap<>(LifeCycle.class)**, toSet())));
...
**// Adding a new phase using the nested EnumMap implementation** public enum Phase { SOLID, LIQUID, GAS**, PLASMA**; public enum Transition { MELT(SOLID, LIQUID), FREEZE(LIQUID, SOLID), BOIL(LIQUID, GAS), CONDENSE(GAS, LIQUID), SUBLIME(SOLID, GAS), DEPOSIT(GAS, SOLID)**, IONIZE(GAS, PLASMA), DEIONIZE(PLASMA, GAS)**; ... // Remainder unchanged } }
...
#### <span id="page-196-0"></span>**[Item 38: Emulate extensible enums with interfaces](#page-8-0)**
...
<span id="page-199-0"></span>The resulting code is a bit less complex, and the test method is a bit more flexible: it allows the caller to combine operations from multiple implementation types. On the other hand, you forgo the ability to use EnumSet (Item 36) and EnumMap (Item 37) on the specified operations.
...
#### <span id="page-200-0"></span>**[Item 39: Prefer annotations to naming patterns](#page-8-0)**
...
If an attempt to invoke a test method by reflection throws any exception other than InvocationTargetException, it indicates an invalid use of the Test annotation that was not caught at compile time. Such uses include annotation of an instance method, of a method with one or more parameters, or of an inaccessible method. The second catch block in the test runner catches these Test usage errors <span id="page-203-0"></span>and prints an appropriate error message. Here is the output that is printed if RunTests is run on Sample:
...
<span id="page-206-0"></span>takes a single parameter, which is the class object of a *containing annotation type*, whose sole parameter is an array of the annotation type [JLS, 9.6.3]. Here's how the annotation declarations look if we take this approach with our ExceptionTest annotation. Note that the containing annotation type must be annotated with an appropriate retention policy and target, or the declarations won't compile:
...
Processing repeatable annotations requires care. A repeated annotation generates a synthetic annotation of the containing annotation type. The getAnnotationsByType method glosses over this fact, and can be used to access both repeated and non-repeated annotations of a repeatable annotation type. But isAnnotationPresent makes it explicit that repeated annotations are not of the annotation type, but of the containing annotation type. If an element has a repeated annotation of some type and you use the isAnnotationPresent method to check if the element has an annotation of that type, you'll find that it does not. Using this method to check for the presence of an annotation type will therefore cause your program to silently ignore repeated annotations. Similarly, using this method to check for the containing annotation type will cause the program to silently ignore non-repeated annotations. To detect repeated and non-repeated annotations with isAnnotationPresent, you much check for both the annotation type and its containing annotation type. Here's how the relevant part of our <span id="page-207-0"></span>RunTests program looks when modified to use the repeatable version of the ExceptionTest annotation:
...
#### <span id="page-208-0"></span>**[Item 40: Consistently use the](#page-8-0) Override annotation**
...
<span id="page-210-0"></span>Most IDEs provide another reason to use the Override annotation consistently. If you enable the appropriate check, the IDE will generate a warning if you have a method that doesn't have an Override annotation but does override a superclass method. If you use the Override annotation consistently, these warnings will alert you to unintentional overriding. They complement the compiler's error messages, which alert you to unintentional failure to override. Between the IDE and the compiler, you can be sure that you're overriding methods everywhere you want to and nowhere else.
...
### <span id="page-211-0"></span>**[Item 41: Use marker interfaces to define types](#page-8-0)**
...
<span id="page-215-0"></span>**// Enum type with constant-specific class bodies & data (Item 34)** public enum Operation { PLUS("+") { public double apply(double x, double y) { return x + y; } }, MINUS("-") { public double apply(double x, double y) { return x - y; } }, TIMES("\*") { public double apply(double x, double y) { return x \* y; } }, DIVIDE("/") { public double apply(double x, double y) { return x / y; } }; private final String symbol; Operation(String symbol) { this.symbol = symbol; } @Override public String toString() { return symbol; } public abstract double apply(double x, double y); }
...
### <span id="page-217-0"></span>**[Item 43: Prefer method references to lambdas](#page-8-0)**
...
If you're programming with an IDE, it will offer to replace a lambda with a method reference wherever it can. You should usually, but not always, take the IDE up on the offer. Occasionally, a lambda will be more succinct than a method reference. This happens most often when the method is in the same class as the <span id="page-218-0"></span>lambda. For example, consider this snippet, which is presumed to occur in a class named GoshThisClassNameIsHumongous:
...
### <span id="page-219-0"></span>**[Item 44: Favor the use of standard functional interfaces](#page-8-0)**
...
### <span id="page-223-0"></span>**[Item 45: Use streams judiciously](#page-8-0)**
...
<span id="page-227-0"></span>When you start using streams, you may feel the urge to convert all your loops into streams, but resist the urge. While it may be possible, it will likely harm the readability and maintainability of your code base. As a rule, even moderately complex tasks are best accomplished using some combination of streams and iteration, as illustrated by the Anagrams programs above. So **refactor existing code to use streams and use them in new code only where it makes sense to do so.**
...
<span id="page-228-0"></span>For example, let's write a program to print the first twenty *Mersenne primes*. To refresh your memory, a *Mersenne number* is a number of the form 2*<sup>p</sup>* − 1. If *p* is prime, the corresponding Mersenne number *may* be prime; if so, it's a Mersenne prime. As the initial stream in our pipeline, we want all the prime numbers. Here's a method to return that (infinite) stream. We assume a static import has been used for easy access to the static members of BigInteger:
...
There are plenty of tasks where it is not obvious whether to use streams or iteration. For example, consider the task of initializing a new deck of cards. Assume that Card is an immutable value class that encapsulates a Rank and a Suit, both of which are enum types. This task is representative of any task that <span id="page-229-0"></span>requires computing all the pairs of elements that can be chosen from two sets. Mathematicians call this the *Cartesian product* of the two sets. Here's an iterative implementation with a nested for-each loop that should look very familiar to you:
...
#### <span id="page-230-0"></span>**[Item 46: Prefer side-effect-free functions in streams](#page-8-0)**
...
The only tricky part of this code is the comparator that we pass to sorted, comparing(freq::get).reversed(). The comparing method is a comparator construction method (Item 14) that takes a key extraction function. The function takes a word, and the "extraction" is actually a table lookup: the bound method reference freq::get looks up the word in the frequency table and returns the number of times the word appears in the file. Finally, we call reversed on the <span id="page-232-0"></span>comparator, so we're sorting the words from most frequent to least frequent. Then it's a simple matter to limit the stream to ten words and collect them into a list.
...
**// Collector to impose last-write-wins policy** toMap(keyMapper, valueMapper, (v1, v2) -> v2)
...
There are also variant forms of the first three versions of toMap, named toConcurrentMap, that run efficiently in parallel and produce ConcurrentHash-Map instances.
...
If you want groupingBy to return a collector that produces a map with values other than lists, you can specify a *downstream collector* in addition to a classifier. A downstream collector produces a value from a stream containing all the <span id="page-234-0"></span>elements in a category. The simplest use of this parameter is to pass toSet(), which results in a map whose values are sets of elements rather than lists.
...
#### <span id="page-236-0"></span>**[Item 47: Prefer Collection to Stream as a return type](#page-8-0)**
...
<span id="page-245-0"></span>As a simple example of a stream pipeline where parallelism is effective, consider this function for computing π(*n*), the number of primes less than or equal to *n*:
...
On my machine, it takes 31 seconds to compute π(10<sup>8</sup> ) using this function. Simply adding a parallel() call reduces the time to 9.2 seconds:
...
For public and protected methods, use the Javadoc @throws tag to document the exception that will be thrown if a restriction on parameter values is violated <span id="page-248-0"></span>(Item 74). Typically, the resulting exception will be IllegalArgumentException, IndexOutOfBoundsException, or NullPointerException (Item 72). Once you've documented the restrictions on a method's parameters and you've documented the exceptions that will be thrown if these restrictions are violated, it is a simple matter to enforce the restrictions. Here's a typical example:
...
### <span id="page-251-0"></span>**[Item 50: Make defensive copies when needed](#page-9-0)**
...
**// Broken "immutable" time period class** public final class Period { private final Date start; private final Date end; /\*\* \* @param start the beginning of the period \* @param end the end of the period; must not precede start \* @throws IllegalArgumentException if start is after end \* @throws NullPointerException if start or end is null \*/ public Period(Date start, Date end) { if (start.compareTo(end) > 0) throw new IllegalArgumentException( start + " after " + end); this.start = start; this.end = end; } public Date start() { return start; }

<span id="page-252-0"></span>public Date end() { return end; } ... // Remainder omitted }
...
With the new constructor in place, the previous attack will have no effect on the Period instance. Note that **defensive copies are made** *before* **checking the validity of the parameters (Item 49), and the validity check is performed on the copies rather than on the originals.** While this may seem unnatural, it is necessary. It protects the class against changes to the parameters from another <span id="page-253-0"></span>thread during the *window of vulnerability* between the time the parameters are checked and the time they are copied. In the computer security community, this is known as a *time-of-check/time-of-use* or *TOCTOU* attack [Viega01].
...
<span id="page-255-0"></span>promises that it will no longer modify the object directly. A method or constructor that expects to take ownership of a client-provided mutable object must make this clear in its documentation.
...
#### <span id="page-256-0"></span>**[Item 51: Design method signatures carefully](#page-9-0)**
...
#### <span id="page-258-0"></span>**[Item 52: Use overloading judiciously](#page-9-0)**
...
<span id="page-264-0"></span>While the resulting overloading clearly violates the guidelines in this item, it causes no harm because both overloaded methods do exactly the same thing when they are invoked on the same object reference. The programmer may not know which overloading will be invoked, but it is of no consequence so long as they behave identically. The standard way to ensure this behavior is to have the more specific overloading forward to the more general:
...
#### <span id="page-265-0"></span>**[Item 53: Use varargs judiciously](#page-9-0)**
...
Luckily there's a much better way to achieve the desired effect. Declare the method to take two parameters, one normal parameter of the specified type and <span id="page-266-0"></span>one varargs parameter of this type. This solution corrects all the deficiencies of the previous one:
...
### <span id="page-267-0"></span>**[Item 54: Return empty collections or arrays, not nulls](#page-9-0)**
...
In the unlikely event that you have evidence suggesting that allocating empty collections is harming performance, you can avoid the allocations by returning the <span id="page-268-0"></span>same *immutable* empty collection repeatedly, as immutable objects may be shared freely (Item 17). Here is the code to do it, using the Collections.emptyList method. If you were returning a set, you'd use Collections.emptySet; if you were returning a map, you'd use Collections.emptyMap. But remember, this is an optimization, and it's seldom called for. If you think you need it, measure performance before and after, to ensure that it's actually helping:
...
**// Optimization - avoids allocating empty arrays** private static final Cheese[] EMPTY\_CHEESE\_ARRAY = new Cheese[0]; public Cheese[] getCheeses() { return cheesesInStock.toArray(EMPTY\_CHEESE\_ARRAY); }
...
#### <span id="page-269-0"></span>**[Item 55: Return optionals judiciously](#page-9-0)**
...
**// Using optional when you know there's a return value** Element lastNobleGas = max(Elements.NOBLE\_GASES)**.get()**;
...
<span id="page-272-0"></span>For example, consider this code snippet, which prints the process ID of the parent of a process, or N/A if the process has no parent. The snippet uses the ProcessHandle class, introduced in Java 9:
...
#### <span id="page-274-0"></span>**[Item 56: Write doc comments for all exposed API elements](#page-9-0)**
...
## [General Programming](#page-9-0)

<span id="page-281-0"></span>**T**HIS chapter is devoted to the nuts and bolts of the language. It discusses local variables, control structures, libraries, data types, and two extralinguistic facilities: *reflection* and *native methods*. Finally, it discusses optimization and naming conventions.

### **[Item 57: Minimize the scope of local variables](#page-9-0)**
...
**Nearly every local variable declaration should contain an initializer.** If you don't yet have enough information to initialize a variable sensibly, you should <span id="page-282-0"></span>postpone the declaration until you do. One exception to this rule concerns trycatch statements. If a variable is initialized to an expression whose evaluation can throw a checked exception, the variable must be initialized inside a try block (unless the enclosing method can propagate the exception). If the value must be used outside of the try block, then it must be declared before the try block, where it cannot yet be "sensibly initialized." For an example, see page 283.
...
**// Preferred idiom for iterating over a collection or array** for (Element e : c) { ... // Do Something with e }
...
**// Idiom for iterating when you need the iterator** for (Iterator<Element> i = c.iterator(); i.hasNext(); ) { Element e = i.next(); ... // Do something with e and i }
...
The second loop contains a copy-and-paste error: it initializes a new loop variable, i2, but uses the old one, i, which is, unfortunately, still in scope. The resulting <span id="page-283-0"></span>code compiles without error and runs without throwing an exception, but it does the wrong thing. Instead of iterating over c2, the second loop terminates immediately, giving the false impression that c2 is empty. Because the program errs silently, the error can remain undetected for a long time.
...
#### <span id="page-284-0"></span>**[Item 58: Prefer for-each loops to traditional](#page-9-0) for loops**
...
<span id="page-286-0"></span>If instead you use a nested for-each loop, the problem simply disappears. The resulting code is as succinct as you could wish for:
...
#### <span id="page-287-0"></span>**[Item 59: Know and use the libraries](#page-9-0)**
...
public static void main(String[] args) { int n = 2 \* (Integer.MAX\_VALUE / 3); int low = 0; for (int i = 0; i < 1000000; i++) if (random(n) < n/2) low++; System.out.println(low); }
...
#### <span id="page-290-0"></span>**Item 60: Avoid float and double [if exact answers are required](#page-9-0)**
...
<span id="page-291-0"></span>If you run the program, you'll find that you can afford three pieces of candy, and you have \$0.3999999999999999 left. This is the wrong answer! The right way to solve this problem is to **use BigDecimal, int, or long for monetary calculations**.
...
public static void main(String[] args) { final BigDecimal TEN\_CENTS = new BigDecimal(".10"); int itemsBought = 0; BigDecimal funds = **new BigDecimal("1.00")**; for (BigDecimal price = TEN\_CENTS; funds.compareTo(price) >= 0; price = price.add(TEN\_CENTS)) { funds = funds.subtract(price); itemsBought++; } System.out.println(itemsBought + " items bought."); System.out.println("Money left over: \$" + funds); }
...
<span id="page-295-0"></span>Finally, consider the program from page 24 in Item 6:

**// Hideously slow program! Can you spot the object creation?** public static void main(String[] args) { Long sum = 0L; for (long i = 0; i < Integer.MAX\_VALUE; i++) { sum += i; } System.out.println(sum); }
...
#### <span id="page-296-0"></span>**[Item 62: Avoid strings where other types are more appropriate](#page-9-0)**
...
**Strings are poor substitutes for capabilities.** Occasionally, strings are used to grant access to some functionality. For example, consider the design of a thread-local variable facility. Such a facility provides variables for which each thread has its own value. The Java libraries have had a thread-local variable facility since release 1.2, but prior to that, programmers had to roll their own. When confronted with the task of designing such a facility many years ago, several <span id="page-297-0"></span>people independently came up with the same design, in which client-provided string keys are used to identify each thread-local variable:
...
While this solves both of the problems with the string-based API, you can do much better. You don't really need the static methods anymore. They can instead become instance methods on the key, at which point the key is no longer a key for a thread-local variable: it *is* a thread-local variable. At this point, the top-level <span id="page-298-0"></span>class isn't doing anything for you anymore, so you might as well get rid of it and rename the nested class to ThreadLocal:
...
#### <span id="page-299-0"></span>**[Item 63: Beware the performance of string concatenation](#page-9-0)**
...
**// Inappropriate use of string concatenation - Performs poorly!** public String statement() { String result = ""; for (int i = 0; i < numItems(); i++) result += lineForItem(i); // String concatenation return result; }
...
public String statement() { StringBuilder b = new StringBuilder(numItems() \* LINE\_WIDTH); for (int i = 0; i < numItems(); i++) b.append(lineForItem(i)); return b.toString(); }
...
#### <span id="page-300-0"></span>**[Item 64: Refer to objects by their interfaces](#page-9-0)**
...
<span id="page-301-0"></span>Changing the HashMap to a LinkedHashMap will provide predictable iteration order with performance comparable to that of HashMap, without making any special demands on the key type.
...
#### <span id="page-302-0"></span>**[Item 65: Prefer interfaces to reflection](#page-9-0)**
...
For example, here is a program that creates a Set<String> instance whose class is specified by the first command line argument. The program inserts the <span id="page-303-0"></span>remaining command line arguments into the set and prints it. Regardless of the first argument, the program prints the remaining arguments with duplicates eliminated. The order in which these arguments are printed, however, depends on the class specified in the first argument. If you specify java.util.HashSet, they're printed in apparently random order; if you specify java.util.TreeSet, they're printed in alphabetical order because the elements in a TreeSet are sorted:
...
### <span id="page-305-0"></span>**[Item 66: Use native methods judiciously](#page-9-0)**
...
### <span id="page-306-0"></span>**[Item 67: Optimize judiciously](#page-9-0)**
...
### <span id="page-309-0"></span>**[Item 68: Adhere to generally accepted naming conventions](#page-9-0)**
...
Class and interface names, including enum and annotation type names, should consist of one or more words, with the first letter of each word capitalized, for example, List or FutureTask. Abbreviations are to be avoided, except for acronyms and certain common abbreviations like max and min. There is some disagreement as to whether acronyms should be uppercase or have only their first letter capitalized. While some programmers still use uppercase, a strong argument <span id="page-310-0"></span>can be made in favor of capitalizing only the first letter: even if multiple acronyms occur back-to-back, you can still tell where one word starts and the next word ends. Which class name would you rather see, HTTPURL or HttpUrl?
...
if (car.speed() > 2 \* SPEED\_LIMIT) generateAudibleAlert("Watch out for cops!");
...
<span id="page-313-0"></span>**W**HEN used to best advantage, exceptions can improve a program's readability, reliability, and maintainability. When used improperly, they can have the opposite effect. This chapter provides guidelines for using exceptions effectively.

### **[Item 69: Use exceptions only for exceptional conditions](#page-9-0)**
...
### <span id="page-316-0"></span>**[Item 70: Use checked exceptions for recoverable conditions and](#page-9-0) runtime exceptions for programming errors**
...
#### <span id="page-318-0"></span>**[Item 71: Avoid unnecessary use of checked exceptions](#page-9-0)**
...
} catch (TheCheckedException e) { throw new AssertionError(); // Can't happen! } Or this? } catch (TheCheckedException e) { e.printStackTrace(); // Oh well, we lose. System.exit(1); }
...
<span id="page-319-0"></span>You can also turn a checked exception into an unchecked exception by breaking the method that throws the exception into two methods, the first of which returns a boolean indicating whether the exception would be thrown. This API refactoring transforms the calling sequence from this:
...
#### <span id="page-320-0"></span>**[Item 72: Favor the use of standard exceptions](#page-9-0)**
...
<span id="page-321-0"></span>**Do** *not* **reuse Exception, RuntimeException, Throwable, or Error directly.** Treat these classes as if they were abstract. You can't reliably test for these exceptions because they are superclasses of other exceptions that a method may throw.
...
#### <span id="page-322-0"></span>**[Item 73: Throw exceptions appropriate to the abstraction](#page-9-0)**
...
A special form of exception translation called *exception chaining* is called for in cases where the lower-level exception might be helpful to someone debugging the problem that caused the higher-level exception. The lower-level exception (the <span id="page-323-0"></span>*cause*) is passed to the higher-level exception, which provides an accessor method (Throwable's getCause method) to retrieve the lower-level exception:
...
#### <span id="page-324-0"></span>**[Item 74: Document all exceptions thrown by each method](#page-9-0)**
...
<span id="page-325-0"></span>It should be noted that documenting all of the unchecked exceptions that each method can throw is an ideal, not always achievable in the real world. When a class undergoes revision, it is not a violation of source or binary compatibility if an exported method is modified to throw additional unchecked exceptions. Suppose a class invokes a method from another, independently written class. The authors of the former class may carefully document all of the unchecked exceptions that each method throws, but if the latter class is revised to throw additional unchecked exceptions, it is quite likely that the former class (which has not undergone revision) will propagate the new unchecked exceptions even though it does not document them.
...
#### <span id="page-326-0"></span>**[Item 75: Include failure-capture information in detail messages](#page-9-0)**
...
#### <span id="page-328-0"></span>**[Item 76: Strive for failure atomicity](#page-9-0)**
...
A closely related approach to achieving failure atomicity is to order the computation so that any part that may fail takes place before any part that modifies the object. This approach is a natural extension of the previous one when arguments cannot be checked without performing a part of the computation. For example, consider the case of TreeMap, whose elements are sorted according to some ordering. In order to add an element to a TreeMap, the element must be of a type that can be compared using the TreeMap's ordering. Attempting to add an incorrectly <span id="page-329-0"></span>typed element will naturally fail with a ClassCastException as a result of searching for the element in the tree, before the tree has been modified in any way.
...
#### <span id="page-330-0"></span>**[Item 77: Don't ignore exceptions](#page-9-0)**
...
<span id="page-333-0"></span>The problem is that in the absence of synchronization, there is no guarantee as to when, if ever, the background thread will see the change in the value of stopRequested made by the main thread. In the absence of synchronization, it's quite acceptable for the virtual machine to transform this code:
...
<span id="page-335-0"></span>The intent of the method is to guarantee that every invocation returns a unique value (so long as there are no more than 232 invocations). The method's state consists of a single atomically accessible field, nextSerialNumber, and all possible values of this field are legal. Therefore, no synchronization is necessary to protect its invariants. Still, the method won't work properly without synchronization.
...
### <span id="page-337-0"></span>**[Item 79: Avoid excessive synchronization](#page-10-0)**
...
@Override public boolean add(E element) { boolean added = super.add(element); if (added) notifyElementAdded(element); return added; } @Override public boolean addAll(Collection<? extends E> c) { boolean result = false; for (E element : c) result |= add(element); // Calls notifyElementAdded return result; } }
...
<span id="page-340-0"></span>Incidentally, note that this program catches two different exception types in one catch clause. This facility, informally known as *multi-catch*, was added in Java 7. It can greatly increase the clarity and reduce the size of programs that behave the same way in response to multiple exception types.
...
The first part of this item was about correctness. Now let's take a brief look at performance. While the cost of synchronization has plummeted since the early days of Java, it is more important than ever not to oversynchronize. In a multicore world, the real cost of excessive synchronization is not the CPU time spent getting locks; it is *contention*: the lost opportunities for parallelism and the delays <span id="page-342-0"></span>imposed by the need to ensure that every core has a consistent view of memory. Another hidden cost of oversynchronization is that it can limit the VM's ability to optimize code execution.
...
### <span id="page-343-0"></span>**[Item 80: Prefer executors, tasks, and streams to threads](#page-10-0)**
...
If you want more than one thread to process requests from the queue, simply call a different static factory that creates a different kind of executor service called a *thread pool*. You can create a thread pool with a fixed or variable number of threads. The java.util.concurrent.Executors class contains static factories that provide most of the executors you'll ever need. If, however, you want some<span id="page-344-0"></span>thing out of the ordinary, you can use the ThreadPoolExecutor class directly. This class lets you configure nearly every aspect of a thread pool's operation.
...
### <span id="page-345-0"></span>**[Item 81: Prefer concurrency utilities to](#page-10-0) wait and notify**
...
<span id="page-346-0"></span>**// Concurrent canonicalizing map atop ConcurrentMap - faster!** public static String intern(String s) { String result = map.get(s); if (result == null) { result = map.putIfAbsent(s, s); if (result == null) result = s; } return result; }
...
- <span id="page-349-0"></span>• Another thread could have obtained the lock and changed the guarded state between the time a thread invoked notify and the waiting thread woke up.
...
#### <span id="page-350-0"></span>**[Item 82: Document thread safety](#page-10-0)**
...
<span id="page-351-0"></span>thread-hostile class on purpose; such classes typically result from the failure to consider concurrency. When a class or method is found to be thread-hostile, it is typically fixed or deprecated. The generateSerialNumber method in Item 78 would be thread-hostile in the absence of internal synchronization, as discussed on page 322.
...
<span id="page-352-0"></span>To prevent this denial-of-service attack, you can use a *private lock object* instead of using synchronized methods (which imply a publicly accessible lock):
...
### <span id="page-353-0"></span>**[Item 83: Use lazy initialization judiciously](#page-10-0)**
...
<span id="page-354-0"></span>Both of these idioms (*normal initialization* and *lazy initialization with a synchronized accessor*) are unchanged when applied to static fields, except that you add the static modifier to the field and accessor declarations.
...
#### <span id="page-356-0"></span>**[Item 84: Don't depend on the thread scheduler](#page-10-0)**
...
**// Awful CountDownLatch implementation - busy-waits incessantly!** public class SlowCountDownLatch { private int count; public SlowCountDownLatch(int count) { if (count < 0) throw new IllegalArgumentException(count + " < 0"); this.count = count; } public void await() { **while (true) { synchronized(this) { if (count == 0) return; } }** }

<span id="page-357-0"></span>public synchronized void countDown() { if (count != 0) count--; } }
...
<span id="page-360-0"></span>The attack surface includes classes in the Java platform libraries, in third-party libraries such as Apache Commons Collections, and in the application itself. Even if you adhere to all of the relevant best practices and succeed in writing serializable classes that are invulnerable to attack, your application may still be vulnerable. To quote Robert Seacord, technical manager of the CERT Coordination Center:
...
The most significant differences between JSON and protobuf are that JSON is text-based and human-readable, whereas protobuf is binary and substantially more <span id="page-362-0"></span>efficient; and that JSON is exclusively a data representation, whereas protobuf offers *schemas* (types) to document and enforce appropriate usage. Although protobuf is more efficient than JSON, JSON is extremely efficient for a text-based representation. And while protobuf is a binary representation, it does provide an alternative text representation for use where human-readability is desired (pbtxt).
...
### <span id="page-363-0"></span>**[Item 86: Implement](#page-10-0) Serializable with great caution**
...
<span id="page-365-0"></span>Classes designed for inheritance that do implement Serializable include Throwable and Component. Throwable implements Serializable so RMI can send exceptions from server to client. Component implements Serializable so GUIs can be sent, saved, and restored, but even in the heyday of Swing and AWT, this facility was little-used in practice.
...
#### <span id="page-366-0"></span>**[Item 87: Consider using a custom serialized form](#page-10-0)**
...
**// Good candidate for default serialized form** public class Name implements Serializable { /\*\* \* Last name. Must be non-null. \* @serial \*/ private final String lastName; /\*\* \* First name. Must be non-null. \* @serial \*/ private final String firstName;

<span id="page-367-0"></span>/\*\* \* Middle name, or null if there is none. \* @serial \*/ private final String middleName; ... // Remainder omitted }
...
**// Awful candidate for default serialized form** public final class StringList implements Serializable { private int size = 0; private Entry head = null; private static class Entry implements Serializable { String data; Entry next; Entry previous; } ... // Remainder omitted }
...
#### <span id="page-368-0"></span>**Using the default serialized form when an object's physical representation differs substantially from its logical data content has four disadvantages:**
...
<span id="page-371-0"></span>Whether or not you accept the default serialized form, every instance field that isn't labeled transient will be serialized when the defaultWriteObject method is invoked. Therefore, every instance field that can be declared transient should be. This includes derived fields, whose values can be computed from primary data fields, such as a cached hash value. It also includes fields whose values are tied to one particular run of the JVM, such as a long field representing a pointer to a native data structure. **Before deciding to make a field nontransient, convince yourself that its value is part of the logical state of the object.** If you use a custom serialized form, most or all of the instance fields should be labeled transient, as in the StringList example above.
...
**// writeObject for synchronized class with default serialized form** private **synchronized** void writeObject(ObjectOutputStream s) throws IOException { s.defaultWriteObject(); }
...
<span id="page-372-0"></span>Declaring a serial version UID is simple. Just add this line to your class:
...
#### <span id="page-373-0"></span>**Item 88: Write readObject [methods defensively](#page-10-0)**
...
<span id="page-374-0"></span>Loosely speaking, readObject is a constructor that takes a byte stream as its sole parameter. In normal use, the byte stream is generated by serializing a normally constructed instance. The problem arises when readObject is presented with a byte stream that is artificially constructed to generate an object that violates the invariants of its class. Such a byte stream can be used to create an *impossible object*, which could not have been created using a normal constructor.
...
<span id="page-377-0"></span>In my locale, running this program produces the following output:
...
#### <span id="page-379-0"></span>**[Item 89: For instance control, prefer enum types to](#page-10-0) readResolve**
...
This method ignores the deserialized object, returning the distinguished Elvis instance that was created when the class was initialized. Therefore, the serialized form of an Elvis instance need not contain any real data; all instance fields should be declared transient. In fact, **if you depend on readResolve for instance** <span id="page-380-0"></span>**control, all instance fields with object reference types** *must* **be declared transient.** Otherwise, it is possible for a determined attacker to secure a reference to the deserialized object before its readResolve method is run, using a technique that is somewhat similar to the MutablePeriod attack in Item 88.
...
<span id="page-382-0"></span>Running this program produces the following output, conclusively proving that it's possible to create two distinct Elvis instances (with different tastes in music):
...
### <span id="page-383-0"></span>**[Item 90: Consider serialization proxies instead of serialized](#page-10-0) instances**
...
<span id="page-384-0"></span>The presence of this method on the enclosing class causes the serialization system to emit a SerializationProxy instance instead of an instance of the enclosing class. In other words, the writeReplace method translates an instance of the enclosing class to its serialization proxy prior to serialization.
...
## <span id="page-387-0"></span>[Items Corresponding to Second Edition](#page-10-0)
...
<span id="page-391-0"></span>[Asserts] *Programming with Assertions*. 2002. Sun Microsystems. [http://docs.oracle.com/javase/8/docs/technotes/guides/language](http://www.docs.oracle.com/javase/8/docs/technotes/guides/language/assert.html) [/assert.html](http://www.docs.oracle.com/javase/8/docs/technotes/guides/language/assert.html) [Beck04] Beck, Kent. 2004. *JUnit Pocket Guide*. Sebastopol, CA: O'Reilly Media, Inc. ISBN: 0596007434. [Bloch01] Bloch, Joshua. 2001. *Effective Java Programming Language Guide.* Boston: Addison-Wesley. ISBN: 0201310058. [Bloch05] Bloch, Joshua, and Neal Gafter. 2005. *Java Puzzlers: Traps, Pitfalls, and Corner Cases.* Boston: Addison-Wesley. ISBN: 032133678X. [Blum14] Blum, Scott. 2014. "Faster RSA in Java with GMP." *The Square Corner* (blog). Feb. 14, 2014. [https://medium.com/square-corner](https://www.medium.com/square-corner-blog/faster-rsa-in-java-with-gmp-8b13c51c6ec4) [-blog/faster-rsa-in-java-with-gmp-8b13c51c6ec4](https://www.medium.com/square-corner-blog/faster-rsa-in-java-with-gmp-8b13c51c6ec4) [Bracha04] Bracha, Gilad. 2004. "Lesson: Generics" online supplement to *The Java Tutorial: A Short Course on the Basics,* 6th ed. Upper Saddle River, NJ: Addison-Wesley, 2014. [https://docs.oracle.com/javase](https://www.docs.oracle.com/javase/tutorial/extra/generics/) [/tutorial/extra/generics/](https://www.docs.oracle.com/javase/tutorial/extra/generics/) [Burn01] Burn, Oliver. 2001–2017. *Checkstyle.* [http://checkstyle.sourceforge.net](http://www.checkstyle.sourceforge.net)

[Coekaerts15] Coekaerts, Wouter (@WouterCoekaerts). 2015. "Billion-laughsstyle DoS for Java serialization [https://gist.github.com/coekie/](https://www.gist.github.com/coekie/a27cc406fc9f3dc7a70d) [a27cc406fc9f3dc7a70d](https://www.gist.github.com/coekie/a27cc406fc9f3dc7a70d) … WONTFIX," Twitter, November 9, 2015, 9:46 a.m. [https://twitter.com/woutercoekaerts/status](https://www.twitter.com/woutercoekaerts/status/663774695381078016) [/663774695381078016](https://www.twitter.com/woutercoekaerts/status/663774695381078016) [CompSci17] Brief of Computer Scientists as Amici Curiae for the United States Court of Appeals for the Federal Circuit, Case No. 17-1118, Oracle America, Inc. v. Google, Inc. in Support of Defendant-Appellee. (2017) [Dagger] *Dagger*. 2013. Square, Inc. [http://square.github.io/dagger/](http://www.square.github.io/dagger/) [Gallagher16] Gallagher, Sean. 2016. "Muni system hacker hit others by scanning for year-old Java vulnerability." *Ars Technica,* November 29, 2016. [https://arstechnica.com/information-technology/2016/11/san](https://www.arstechnica.com/information-technology/2016/11/san-francisco-transit-ransomware-attacker-likely-used-year-old-java-exploit/) [-francisco-transit-ransomware-attacker-likely-used-year-old-java](https://www.arstechnica.com/information-technology/2016/11/san-francisco-transit-ransomware-attacker-likely-used-year-old-java-exploit/) [-exploit/](https://www.arstechnica.com/information-technology/2016/11/san-francisco-transit-ransomware-attacker-likely-used-year-old-java-exploit/) [Gamma95] Gamma, Erich, Richard Helm, Ralph Johnson, and John Vlissides. 1995. *Design Patterns: Elements of Reusable Object-Oriented Software.* Reading, MA: Addison-Wesley. ISBN: 0201633612. [Goetz06] Goetz, Brian. 2006. *Java Concurrency in Practice*. With Tim Peierls, Joshua Bloch, Joseph Bowbeer, David Holmes, and Doug Lea. Boston: Addison-Wesley. ISBN: 0321349601. [Gosling97] Gosling, James. 1997. "The Feel of Java." *Computer* 30 no. 6 (June 1997): 53-57. [http://dx.doi.org/10.1109/2.587548](http://www.dx.doi.org/10.1109/2.587548) [Guava] *Guava*. 2017. Google Inc. [https://github.com/google/guava](https://www.github.com/google/guava) [Guice] *Guice*. 2006. Google Inc. [https://github.com/google/guice](https://www.github.com/google/guice) [Herlihy12] Herlihy, Maurice, and Nir Shavit. 2012. *The Art of Multiprocessor Programming, Revised Reprint.* Waltham, MA: Morgan Kaufmann Publishers. ISBN: 0123973376.

[Jackson75] Jackson, M. A. 1975. *Principles of Program Design.* London: Academic Press. ISBN: 0123790506. [Java-secure] *Secure Coding Guidelines for Java SE*. 2017. Oracle. [http://](http://www.oracle.com/technetwork/java/seccodeguide-139067.html) [www.oracle.com/technetwork/java/seccodeguide-139067.html](http://www.oracle.com/technetwork/java/seccodeguide-139067.html) [Java8-feat] *What's New in JDK 8*. 2014. Oracle. [http://www.oracle.com](http://www.oracle.com/technetwork/java/javase/8-whats-new-2157071.html) [/technetwork/java/javase/8-whats-new-2157071.html](http://www.oracle.com/technetwork/java/javase/8-whats-new-2157071.html) [Java9-feat] *Java Platform, Standard Edition What's New in Oracle JDK 9*. 2017. Oracle. [https://docs.oracle.com/javase/9/whatsnew/toc.htm](https://www.docs.oracle.com/javase/9/whatsnew/toc.htm) [Java9-api] *Java Platform, Standard Edition & Java Development Kit Version 9 API Specification*. 2017. Oracle. [https://docs.oracle.com](https://www.docs.oracle.com/javase/9/docs/api/overview-summary.html) [/javase/9/docs/api/overview-summary.html](https://www.docs.oracle.com/javase/9/docs/api/overview-summary.html) [Javadoc-guide] *How to Write Doc Comments for the Javadoc Tool*. 2000–2004. Sun Microsystems. [http://www.oracle.com/technetwork/java](http://www.oracle.com/technetwork/java/javase/documentation/index-137868.html) [/javase/documentation/index-137868.html](http://www.oracle.com/technetwork/java/javase/documentation/index-137868.html) [Javadoc-ref] *Javadoc Reference Guide.* 2014-2017. Oracle. [https://docs.oracle.com/javase/9/javadoc/javadoc.htm](https://www.docs.oracle.com/javase/9/javadoc/javadoc.htm) [JLS] Gosling, James, Bill Joy, Guy Steele, and Gilad Bracha. 2014. *The Java Language Specification, Java SE 8 Edition*. Boston: Addison-Wesley. ISBN: 013390069X. [JMH] *Code Tools:* jmh. 2014. Oracle. [http://openjdk.java.net/projects/code-tools/jmh/](http://www.openjdk.java.net/projects/code-tools/jmh/) [JSON] *Introducing JSON*. 2013. Ecma International.<https://www.json.org> [Kahan91] Kahan, William, and J. W. Thomas. 1991. *Augmenting a Programming Language with Complex Arithmetic*. UCB/CSD-91-667, University of California, Berkeley.

[Knuth74] Knuth, Donald. 1974. Structured Programming with go to Statements. In *Computing Surveys* 6: 261–301. [Lea14] Lea, Doug. 2014. *When to use parallel streams*. [http://gee.cs.oswego.edu/dl/html/StreamParallelGuidance.html](http://www.gee.cs.oswego.edu/dl/html/StreamParallelGuidance.html) [Lieberman86] Lieberman, Henry. 1986. Using Prototypical Objects to Implement Shared Behavior in Object-Oriented Systems. In *Proceedings of the First ACM Conference on Object-Oriented Programming Systems, Languages, and Applications*, pages 214–223, Portland, September 1986. ACM Press. [Liskov87] Liskov, B. 1988. Data Abstraction and Hierarchy. In *Addendum to the Proceedings of OOPSLA '87* and *SIGPLAN Notices,* Vol. 23, No. 5: 17–34, May 1988. [Naftalin07] Naftalin, Maurice, and Philip Wadler. 2007. *Java Generics and Collections*. Sebastopol, CA: O'Reilly Media, Inc. ISBN: 0596527756. [Parnas72] Parnas, D. L. 1972. On the Criteria to Be Used in Decomposing Systems into Modules. In *Communications of the ACM* 15: 1053– 1058. [POSIX] 9945-1:1996 (ISO/IEC) [IEEE/ANSI Std. 1003.1 1995 Edition] Information Technology—Portable Operating System Interface (POSIX)—Part 1: System Application: Program Interface (API) C Language] (ANSI), IEEE Standards Press, ISBN: 1559375736. [Protobuf] *Protocol Buffers*. 2017. Google Inc. [https://developers.google.com/protocol-buffers](https://www.developers.google.com/protocol-buffers) [Schneider16] Schneider, Christian. 2016. SWAT (Serial Whitelist Application Trainer). [https://github.com/cschneider4711/SWAT/](https://www.github.com/cschneider4711/SWAT/)

[Seacord17] Seacord, Robert. 2017. *Combating Java Deserialization Vulnerabilities with Look-Ahead Object Input Streams (LAOIS)*. San Francisco: NCC Group Whitepaper. [https://www.nccgroup.trust/globalassets/our-research/us/](https://www.nccgroup.trust/globalassets/our-research/us/whitepapers/2017/june/ncc_group_combating_java_deserialization_vulnerabilities_with_look-ahead_object_input_streams1.pdf) [whitepapers/2017/june/ncc\\_group\\_combating\\_java\\_deserialization](https://www.nccgroup.trust/globalassets/our-research/us/whitepapers/2017/june/ncc_group_combating_java_deserialization_vulnerabilities_with_look-ahead_object_input_streams1.pdf) [\\_vulnerabilities\\_with\\_look-ahead\\_object\\_input\\_streams1.pdf](https://www.nccgroup.trust/globalassets/our-research/us/whitepapers/2017/june/ncc_group_combating_java_deserialization_vulnerabilities_with_look-ahead_object_input_streams1.pdf) [Serialization] *Java Object Serialization Specification*. March 2005. Sun Microsystems. [http://docs.oracle.com/javase/9/docs/specs](http://www.docs.oracle.com/javase/9/docs/specs/serialization/index.html) [/serialization/index.html](http://www.docs.oracle.com/javase/9/docs/specs/serialization/index.html) [Sestoft16] Sestoft, Peter. 2016. *Java Precisely*, 3rd ed*.* Cambridge, MA: The MIT Press. ISBN: 0262529076. [Shipilëv16] Aleksey Shipilëv. 2016. *Arrays of Wisdom of the Ancients.* [https://shipilev.net/blog/2016/arrays-wisdom-ancients/](https://www.shipilev.net/blog/2016/arrays-wisdom-ancients/) [Smith62] Smith, Robert. 1962. Algorithm 116 Complex Division. In *Communications of the ACM* 5, no. 8 (August 1962): 435. [Snyder86] Snyder, Alan. 1986. "Encapsulation and Inheritance in Object-Oriented Programming Languages." In *Object-Oriented Programming Systems, Languages, and Applications Conference Proceedings*, 38–45. New York, NY: ACM Press. [Spring] *Spring Framework*. Pivotal Software, Inc. 2017. [https://projects.spring.io/spring-framework/](https://www.projects.spring.io/spring-framework/) [Stroustrup] Stroustrup, Bjarne. [ca. 2000]. "Is Java the language you would have designed if you didn't have to be compatible with C?" *Bjarne Stroustrup's FAQ*. Updated Ocober 1, 2017. [http://www.stroustrup.com/bs\\_faq.html#Java](http://www.stroustrup.com/bs_faq.html#Java)

[Stroustrup95] Stroustrup, Bjarne. 1995. "Why C++ is not just an object-oriented programming language." In *Addendum to the proceedings of the 10th annual conference on Object-oriented programming systems, languages, and applications*, edited by Steven Craig Bilow and Patricia S. Bilow New York, NY: ACM. [http://dx.doi.org/10.1145/260094.260207](http://www.dx.doi.org/10.1145/260094.260207) [Svoboda16] Svoboda, David. 2016. *Exploiting Java Serialization for Fun and Profit*. Software Engineering Institute, Carnegie Mellon University. [https://resources.sei.cmu.edu/library/asset-view.cfm?assetid=484347](https://www.resources.sei.cmu.edu/library/asset-view.cfm?assetid=484347) [Thomas94] Thomas, Jim, and Jerome T. Coonen. 1994. "Issues Regarding Imaginary Types for C and C++." In *The Journal of C Language Translation* 5, no. 3 (March 1994): 134–138. [ThreadStop] *Why Are* Thread.stop*,* Thread.suspend*,* Thread.resume *and*  Runtime.runFinalizersOnExit *Deprecated?* 1999. Sun Microsystems. [https://docs.oracle.com/javase/8/docs/technotes](https://www.docs.oracle.com/javase/8/docs/technotes/guides/concurrency/threadPrimitiveDeprecation.html) [/guides/concurrency/threadPrimitiveDeprecation.html](https://www.docs.oracle.com/javase/8/docs/technotes/guides/concurrency/threadPrimitiveDeprecation.html) [Viega01] Viega, John, and Gary McGraw. 2001. *Building Secure Software: How to Avoid Security Problems the Right Way.* Boston: Addison-Wesley. ISBN: 020172152X. [W3C-validator]*W3C Markup Validation Service*. 2007. World Wide Web Consortium. [http://validator.w3.org/](http://www.validator.w3.org/) [Wulf72] Wulf, W. A Case Against the GOTO. 1972. In *Proceedings of the 25th ACM National Conference* 2: 791–797. New York, NY: ACM Press.

# [Index](#page-10-0)

#### <span id="page-397-0"></span>**Symbols**

\_ in numeric literals, [108](#page-128-0) ... variable arity argument *See* varargs < > type parameter delimiters, [117](#page-137-0), [123](#page-143-0) <?> unbounded wildcard type, [120](#page-140-0)

#### **A**

abstract classes designing for inheritance, [97](#page-117-0) vs. interfaces, [99](#page-119-0)[–103](#page-123-0) noninstantiability and, [19](#page-39-0) subclassing, and equals, [45](#page-65-0) access control mechanisms, [74](#page-94-0) access levels, [73](#page-93-0)[–77](#page-97-0) module-level, [76](#page-96-0)[–77](#page-97-0) of readResolve, [362](#page-382-0) rules of thumb for, [74](#page-94-0)[–75](#page-95-0) of static member classes, [112](#page-132-0) access modifiers, [74](#page-94-0) accessor methods, [78](#page-98-0) defensive copies and, [80](#page-100-0), [233](#page-253-0) for failure-capture data, [297](#page-317-0), [307](#page-327-0) immutability and, [80](#page-100-0) naming conventions for, [291](#page-311-0)[–292](#page-312-0) vs. public fields, [78](#page-98-0)[–79](#page-99-0) for toString data, [57](#page-77-0) actual type parameters, [117](#page-137-0) Adapter pattern, [23](#page-43-0), [101](#page-121-0), [113](#page-133-0) aggregate types vs. strings, [276](#page-296-0) alien methods, [317](#page-337-0) annotations, [157](#page-177-0), [180](#page-200-0)[–192](#page-212-0) defining, [180](#page-200-0) detecting repeated and non-repeated, [186](#page-206-0) documenting, [259](#page-279-0)

functional interfaces and, [202](#page-222-0) vs. naming patterns, [180](#page-200-0)[–187](#page-207-0) synthetic, [186](#page-206-0) *See also* marker annotations anonymous classes, [112](#page-132-0), [114](#page-134-0), [193](#page-213-0) in adapters, [101](#page-121-0) prefer lambdas to, [193](#page-213-0)[–196](#page-216-0) serialization and, [196](#page-216-0) antipatterns, [2](#page-22-0) bit field enum pattern, [169](#page-189-0) bounded wildcard types as return types, [142](#page-162-0) breaking from naming conventions, [82](#page-102-0) busy waits, [336](#page-356-0) constant as a hashCode, [51](#page-71-0) constant interface, [107](#page-127-0)[–108](#page-128-0) copy constructor of immutable object, [83](#page-103-0) difference-based comparators, [71](#page-91-0) empty catch blocks, [310](#page-330-0) equals with unreliable resources, [45](#page-65-0) exceptions for flow control, [293](#page-313-0) excessive string concatenation, [279](#page-299-0) floating point for monetary calculations, [270](#page-290-0) hardwiring resources, [20](#page-40-0) hashCode missing significant fields, [54](#page-74-0) immutable classes not effectively final, [85](#page-105-0) inappropriate subclassing, [92](#page-112-0) int enum pattern, [157](#page-177-0) naming patterns, [180](#page-200-0) null returns for collections, arrays, [247](#page-267-0)[–248](#page-268-0) ordinal abuse, [168](#page-188-0), [171](#page-191-0) overriding equals but not hashCode, [50](#page-70-0) serializable inner classes, [345](#page-365-0) signatures differ by parameter type order, [6](#page-26-0) String enum pattern, [158](#page-178-0) string overuse, [276](#page-296-0) tagged classes, [109](#page-129-0)[–111](#page-131-0) value-component subclasses and equals, [44](#page-64-0) API design access levels and, [74](#page-94-0) bounded wildcard types and, [139](#page-159-0)[–145](#page-165-0) callbacks, [28](#page-48-0) constant interface pattern and, [107](#page-127-0) exceptions and, [294](#page-314-0), [296](#page-316-0)[–297](#page-317-0) information hiding and, [286](#page-306-0) inheritance and, [93](#page-113-0)[–98](#page-118-0) interfaces as parameter types, [170](#page-190-0) member classes and, [114](#page-134-0) performance and, [286](#page-306-0)[–287](#page-307-0) serialization and, [343](#page-363-0)[–345](#page-365-0) singletons, [18](#page-38-0) API elements, [4](#page-24-0) documenting, [254](#page-274-0)[–260](#page-280-0) API, toString return values as defacto, [57](#page-77-0) arrays clone and, [65](#page-85-0) covariant typing, [126](#page-146-0) defensive copying of, [76](#page-96-0), [234](#page-254-0) empty, vs. null as return value, [247](#page-267-0)[–248](#page-268-0) to implement generics, [131](#page-151-0)[–133](#page-153-0) vs. lists, [126](#page-146-0)[–129](#page-149-0) mutability and, [234](#page-254-0), [248](#page-268-0) reified, [126](#page-146-0) security issues, [76](#page-96-0) assertions, [229](#page-249-0) atomicity of variables, [311](#page-331-0) synchronization and, [312](#page-332-0)[–314](#page-334-0) autoboxing, [24](#page-44-0), [273](#page-293-0)[–275](#page-295-0) performance and, [275](#page-295-0) AutoCloseable interface, [31](#page-51-0)[–32](#page-52-0), [35](#page-55-0) **B** backing objects, [23](#page-43-0) base classes, [281](#page-301-0) BigDecimal class compareTo inconsistent with equals, [68](#page-88-0) for monetary calculations, [270](#page-290-0) performance and, [271](#page-291-0) bit fields vs. enum sets, [169](#page-189-0)[–170](#page-190-0) blocking operations, [326](#page-346-0) bogus byte stream attacks, [354](#page-374-0) bounded type parameters, [134](#page-154-0) for bounded type tokens, [154](#page-174-0) vs. bounded wildcard types, [144](#page-164-0) bounded type tokens, [154](#page-174-0), [172](#page-192-0), [178](#page-198-0), [183](#page-203-0) bounded wildcard types, [136](#page-156-0), [140](#page-160-0) for API flexibility, [139](#page-159-0)[–145](#page-165-0) vs. bounded type parameters, [144](#page-164-0) for bounded type tokens, [154](#page-174-0) vs. class objects, [178](#page-198-0) dependency injection and, [21](#page-41-0) PECS mnemonic for, [141](#page-161-0) as return types, [142](#page-162-0) vs. unbounded wildcard types, [121](#page-141-0) boxed primitives == operator and, [274](#page-294-0) appropriate uses of, [275](#page-295-0) generics and, [134](#page-154-0) prefer primitive types to, [24](#page-44-0), [273](#page-293-0)[–275](#page-295-0) Bridge pattern, [8](#page-28-0) Builder pattern, [10](#page-30-0)[–16](#page-36-0) adapted for method invocation, [237](#page-257-0) busy waits, [336](#page-356-0) **C** caching avoiding memory leaks from, [28](#page-48-0) of expensive objects, [22](#page-42-0)[–23](#page-43-0) of hash codes, [53](#page-73-0) immutable objects and, [82](#page-102-0), [85](#page-105-0) callback frameworks, wrapper classes and, [91](#page-111-0) callbacks, avoiding memory leaks from, [28](#page-48-0) canonical forms, [47](#page-67-0) capabilities vs. strings, [276](#page-296-0)[–277](#page-297-0) casts dynamic, [153](#page-173-0), [155](#page-175-0) invisible (*see* compiler-generated casts) unchecked, warnings of, [127](#page-147-0), [129](#page-149-0), [137](#page-157-0) char values, and streams, [206](#page-226-0) checked exceptions avoiding overuse of, [298](#page-318-0)[–299](#page-319-0) declaring, [304](#page-324-0) failure atomicity and, [308](#page-328-0) purpose of, [296](#page-316-0) refactoring to unchecked, [299](#page-319-0) vs. unchecked, [296](#page-316-0)[–297](#page-317-0)

boolean vs. enum types, [237](#page-257-0)

circularities in cleaners, [33](#page-53-0) initialization, [333](#page-353-0), [366](#page-386-0) serialization attacks and, [360](#page-380-0) Class class, as parameterized key, [151](#page-171-0) class hierarchies, [110](#page-130-0) Builder pattern and, [14](#page-34-0) combinatorial explosions in, [100](#page-120-0) class literals as annotation parameter values, [183](#page-203-0) as generics, [151](#page-171-0) raw types in, [121](#page-141-0) class-based frameworks, [281](#page-301-0) classes, [73](#page-93-0)[–114](#page-134-0) access levels of, [74](#page-94-0) anonymous (*see* anonymous classes) base, [281](#page-301-0) composition, [87](#page-107-0)[–92](#page-112-0) designing for inheritance, [93](#page-113-0)[–98](#page-118-0) documenting for inheritance, [93](#page-113-0)[–94](#page-114-0) thread safety of, [330](#page-350-0)[–332](#page-352-0) generic, [117](#page-137-0) helper, for shortening parameter lists, [237](#page-257-0) hierarchy of (*see* class hierarchies) immutable (*see* immutable objects) implementation inheritance, [87](#page-107-0)[–92](#page-112-0) instances of, [3](#page-23-0) levels of thread safety, [330](#page-350-0) members, [3](#page-23-0) minimizing accessibility of, [73](#page-93-0)[–77](#page-97-0) mutable, and thread-safety, [322](#page-342-0) naming conventions for, [289](#page-309-0)[–291](#page-311-0) nested (*see* nested classes) noninstantiable companions, [7](#page-27-0) reusable forwarding, [89](#page-109-0)[–91](#page-111-0) singletons (*see* singletons) summary descriptions of, [257](#page-277-0) SuppressWarnings annotation and, [124](#page-144-0) tagged, vs. class hierarchies, [109](#page-129-0)[–111](#page-131-0) unintentionally instantiable, [19](#page-39-0) unrelated, [243](#page-263-0) utility (*see* utility classes) wrapper (*see* wrapper classes) *See also individual class names* classifier functions, [213](#page-233-0) cleaners, [29](#page-49-0)[–33](#page-53-0) clone method, [58](#page-78-0)[–65](#page-85-0) arrays and, [65](#page-85-0) as a constructor, [61,](#page-81-0) [96](#page-116-0) vs. copy or conversion constructor, [65](#page-85-0) defensive copies and, [76](#page-96-0), [233](#page-253-0) final fields and, [61](#page-81-0) general contract, [58](#page-78-0)[–59](#page-79-0) immutable objects and, [83](#page-103-0) nonfinal classes and, [233](#page-253-0) nonfinal methods and, [64](#page-84-0) overridable methods and, [96](#page-116-0) thread-safety and, [64](#page-84-0) Cloneable interface, [58](#page-78-0)[–65](#page-85-0) alternatives to, [65](#page-85-0) behavior of, [58](#page-78-0) designing for inheritance and, [64](#page-84-0), [96](#page-116-0) implementing, [64](#page-84-0) collections concurrent, [321](#page-341-0), [325](#page-345-0)[–326](#page-346-0) empty, vs. null as return value, [247](#page-267-0) optionals and, [252](#page-272-0) as return types, vs. streams, [216](#page-236-0)[–221](#page-241-0) collectors, [211](#page-231-0)[–215](#page-235-0) downstream, [213](#page-233-0) Collectors API, organization of, [211](#page-231-0)[–215](#page-235-0) combinatorial explosions, [100](#page-120-0) companion classes mutable, [84](#page-104-0) noninstantiable, [7](#page-27-0) Comparable interface, [66](#page-86-0)[–72](#page-92-0) as consumers in PECS, [143](#page-163-0) recursive type bounds and, [137](#page-157-0) *See also* compareTo method comparator construction methods, [70](#page-90-0), [194](#page-214-0) comparators, [69](#page-89-0) as consumers in PECS, [143](#page-163-0) compareTo method, [66](#page-86-0)[–68](#page-88-0) *See also* Comparable interface compatibility backward, [350](#page-370-0) binary, [107](#page-127-0), [305](#page-325-0) forward, [350](#page-370-0) migration, [119](#page-139-0) source, [305](#page-325-0) unchecked exceptions and, [305](#page-325-0) compiler warnings, types of, [123](#page-143-0)

compiler-generated casts, [117](#page-137-0), [119](#page-139-0), [127](#page-147-0) components, [2](#page-22-0) composition, [8,](#page-28-0) [89](#page-109-0) equals and, [44](#page-64-0) vs. inheritance, [87](#page-107-0)[–92](#page-112-0) conceptual weight, [7](#page-27-0) concurrency, [311](#page-331-0) documenting method behavior for, [330](#page-350-0)[–332](#page-352-0) improving via internal synchronization, [322](#page-342-0) concurrency utilities, [323](#page-343-0)[–329](#page-349-0) vs. wait and notify, [325](#page-345-0)[–329](#page-349-0) concurrent collections, [321](#page-341-0), [325](#page-345-0)[–326](#page-346-0) conditionally thread-safe classes, documenting, [331](#page-351-0) consistency requirements equals, [38](#page-58-0), [45](#page-65-0) hashCode, [50](#page-70-0) consistent with equals, [68](#page-88-0) unreliable resources and, [45](#page-65-0) constant fields, naming conventions for, [290](#page-310-0) constant interfaces, [107](#page-127-0) constant utility classes, [108](#page-128-0) constants, [76](#page-96-0) in anonymous classes, [114](#page-134-0) naming conventions for, [290](#page-310-0) constant-specific behaviors, [162](#page-182-0)[–166](#page-186-0) lambdas for, [195](#page-215-0) constant-specific class bodies, [162](#page-182-0) constant-specific method implementations *See* constant-specific behaviors constructors, [4](#page-24-0) calling overridable methods in, [95](#page-115-0) checking parameters of, [353](#page-373-0) clone as a, [61](#page-81-0) copy and conversion, [65](#page-85-0) default, [19](#page-39-0) defensive copying of parameters, [232](#page-252-0) deserialization as, [344](#page-364-0) establishing invariants, [82](#page-102-0), [86](#page-106-0) noninstantiability and, [19](#page-39-0) private (*see* private constructors) readObject as a, [353](#page-373-0) reflection and, [282](#page-302-0) replacing with static factories, [5](#page-25-0)[–9](#page-29-0) safely overloading, [240](#page-260-0)[–241](#page-261-0) for singletons, [17](#page-37-0)[–18](#page-38-0) summary descriptions of, [257](#page-277-0) SuppressWarnings annotation and, [124](#page-144-0) validity checking parameters of, [229](#page-249-0) contention, synchronization and, [321](#page-341-0) contracts clone, [58](#page-78-0) compareTo, [66](#page-86-0) documentation as, [304](#page-324-0) equals, [38](#page-58-0)[–46](#page-66-0) hashCode, [50](#page-70-0) toString, [55](#page-75-0) corrupted objects, [12](#page-32-0), [30](#page-50-0), [227](#page-247-0), [309](#page-329-0) countdown latches, [326](#page-346-0) covariant arrays, [126](#page-146-0) covariant return typing, [16,](#page-36-0) [60](#page-80-0) creating objects, [5](#page-25-0)[–33](#page-53-0) cross-platform structured-data representations, [341](#page-361-0) custom serialized forms, [346](#page-366-0)[–352](#page-372-0) **D** data consistency maintaining in face of failure, [308](#page-328-0)[–309](#page-329-0) synchronization for, [311](#page-331-0)[–316](#page-336-0) data corruption, [285](#page-305-0), [312](#page-332-0) Date, replacements for, [232](#page-252-0) deadlocks resource ordering, [320](#page-340-0), [351](#page-371-0) thread starvation, [328](#page-348-0) Decorator pattern, [91](#page-111-0) default access *See* package-private access level default constructors, [19](#page-39-0) default implementations, [104](#page-124-0) default methods on interfaces, [99](#page-119-0), [104](#page-124-0)[–105](#page-125-0) default serialized forms, [346](#page-366-0)[–352](#page-372-0) disadvantages of, [348](#page-368-0) defensive copies, [231](#page-251-0)[–235](#page-255-0) of arrays, [234](#page-254-0) builders and, [14](#page-34-0) clone and, [233](#page-253-0) deserialization and, [353](#page-373-0), [357](#page-377-0) documenting, [234](#page-254-0) immutable objects and, [83](#page-103-0)

of mutable internal fields, [233](#page-253-0) of mutable parameters, [232](#page-252-0)[–233](#page-253-0) vs. object reuse, [25](#page-45-0) performance and, [234](#page-254-0) readObject and, [353](#page-373-0), [357](#page-377-0) transfers of control and, [235](#page-255-0) validity checking and, [232](#page-252-0) delegation, [91](#page-111-0) denial-of-service attacks, [331](#page-351-0) dependency injection, [20](#page-40-0)[–21](#page-41-0) derived fields, [47](#page-67-0), [52](#page-72-0) deserialization as a constructor, [344](#page-364-0) singletons and, [18](#page-38-0) deserialization bombs, [340](#page-360-0) deserialization filtering, [342](#page-362-0) destroying objects, [26](#page-46-0)[–33](#page-53-0) detail messages, [306](#page-326-0) diamond operator, [123](#page-143-0) documenting annotation types, [259](#page-279-0) compareTo, [67](#page-87-0) conditional thread safety, [331](#page-351-0) enum types, [258](#page-278-0), [331](#page-351-0) exceptions, [227](#page-247-0), [304](#page-324-0)[–305](#page-325-0) exposed API elements, [254](#page-274-0)[–260](#page-280-0) generics, [258](#page-278-0) hashCode, [54](#page-74-0) for inheritance, [93](#page-113-0)[–94](#page-114-0) methods, [254](#page-274-0)[–255](#page-275-0) multiline code examples, [255](#page-275-0) object state after exceptions, [309](#page-329-0) parameters, [227](#page-247-0) return value of toString, [56](#page-76-0) self-use of overridable methods, [93](#page-113-0), [98](#page-118-0) self-use patterns, [256](#page-276-0) serialized fields, [347](#page-367-0) skeletal implementations, [103](#page-123-0) static factories, [9](#page-29-0) SuppressWarnings annotation, [125](#page-145-0) thread safety, [330](#page-350-0)[–332](#page-352-0) writeObject for serialization, [350](#page-370-0) *See also* Javadoc double for binary floating-point arithmetic, [270](#page-290-0)

when to avoid, [270](#page-290-0)[–272](#page-292-0) dynamic casts, [153](#page-173-0), [155](#page-175-0) **E** effectively immutable objects, [316](#page-336-0) empty arrays, vs. null as return value, [247](#page-267-0)[–248](#page-268-0) encapsulation, [73](#page-93-0), [286](#page-306-0) broken by inheritance, [87](#page-107-0), [94](#page-114-0) broken by serialization, [343](#page-363-0) of data fields, [78](#page-98-0) enclosing instances, [112](#page-132-0) enum types, [157](#page-177-0)[–192](#page-212-0) adding data and behaviors to, [159](#page-179-0)[–161](#page-181-0) vs. bit fields, [169](#page-189-0)[–170](#page-190-0) vs. boolean, [237](#page-257-0) built-in serialization mechanism, [362](#page-382-0) constant-specifics for, [162](#page-182-0)[–166](#page-186-0) documenting, [258](#page-278-0), [331](#page-351-0) extensibility and, [176](#page-196-0)[–179](#page-199-0) immutability of, [160](#page-180-0) instance-controlled, [158](#page-178-0) vs. int constants, [157](#page-177-0)[–167](#page-187-0) prefer to readResolve, [359](#page-379-0)[–362](#page-382-0) removing elements from, [161](#page-181-0) singletons from, [17](#page-37-0)[–18](#page-38-0) strategy enum pattern, [166](#page-186-0) vs. String constants, [158](#page-178-0), [276](#page-296-0) switch statements and, [167](#page-187-0) as top-level or member classes, [161](#page-181-0) toString and, [160](#page-180-0) type safety from, [158](#page-178-0) when to use, [167](#page-187-0) enumerated types *See* enum types EnumMap vs. ordinals, [171](#page-191-0)[–175](#page-195-0) EnumSet vs. bit fields, [169](#page-189-0)[–170](#page-190-0) equals method, [37](#page-57-0) accidental overloading of, [49](#page-69-0), [188](#page-208-0) canonical forms and, [47](#page-67-0) compareTo and, [68](#page-88-0) composition and, [44](#page-64-0) general contract for, [38](#page-58-0)[–46](#page-66-0) hashCode and, [48](#page-68-0), [50](#page-70-0)[–54](#page-74-0) how to write, [46](#page-66-0) Override annotation and, [188](#page-208-0)

double-check idiom, [334](#page-354-0)[–335](#page-355-0) downstream collectors, [213](#page-233-0)

equals method (*continued*) return values of, and compareTo, [68](#page-88-0) subclassing and, [42](#page-62-0), [45](#page-65-0) unreliable resources and, [45](#page-65-0) when to override, [37](#page-57-0)[–38](#page-58-0) equivalence classes, [39](#page-59-0) equivalence relations, [38](#page-58-0) erasure, [119](#page-139-0), [126](#page-146-0) errors generic array creation, [126](#page-146-0)[–127](#page-147-0), [133](#page-153-0) purpose of, [297](#page-317-0) runtime, default methods and, [105](#page-125-0) exact results, types for obtaining, [270](#page-290-0) exception chaining, [302](#page-322-0)[–303](#page-323-0) exception translation idiom, [230](#page-250-0), [302](#page-322-0) exceptions, [293](#page-313-0)[–310](#page-330-0) accessor methods for, [297,](#page-317-0) [307](#page-327-0) checked vs. unchecked, [296](#page-316-0)[–297](#page-317-0) choosing among, [301](#page-321-0) commonly reused, [301](#page-321-0) control flow and, [294](#page-314-0) detail messages for, [306](#page-326-0)[–307](#page-327-0) documenting, [227](#page-247-0), [304](#page-324-0)[–305](#page-325-0) failure-capture data, [307](#page-327-0) for invalid method parameters, [228](#page-248-0) ignoring, [310](#page-330-0) logging of, [303](#page-323-0) multi-catch facility, [320](#page-340-0) vs. optionals or special return values, [295](#page-315-0) prefer standard existing, [300](#page-320-0)[–301](#page-321-0) preventing, [303](#page-323-0) vs. state testing methods, [294](#page-314-0) suppression of, [36](#page-56-0) uncaught, and finalizers, [30](#page-50-0) using appropriately, [293](#page-313-0)–[295](#page-315-0) *See also individual exception names* Executor Framework, [323](#page-343-0) executor service, [323](#page-343-0)[–324](#page-344-0) explicit type arguments, [142](#page-162-0) export declarations, [76](#page-96-0) exported APIs *See* API design; APIs extending classes *See* inheritance; subclassing extending interfaces, [4](#page-24-0) extensible enums, [176](#page-196-0)[–179](#page-199-0) *See also* hidden constructors **F** Factory Method pattern, [5](#page-25-0), [21](#page-41-0) failure atomicity, [230](#page-250-0), [308](#page-328-0)[–309](#page-329-0) fields access levels of, [73](#page-93-0)–[77](#page-97-0) class invariants and, [75](#page-95-0) constant, naming conventions for, [290](#page-310-0) derived, [47](#page-67-0), [52](#page-72-0) exposing, vs. accessor methods, [78](#page-98-0)[–79](#page-99-0) final (*see* final fields) initialization techniques for, [335](#page-355-0) mutable, defensive copies of, [233](#page-253-0) naming conventions for, [290](#page-310-0), [292](#page-312-0) public static final, for singletons, [17](#page-37-0) reflection and, [282](#page-302-0) summary descriptions of, [257](#page-277-0) tags, [109](#page-129-0) thread safety and, [75](#page-95-0) final fields for defining constants, [290](#page-310-0) incompatible with cloning, [61](#page-81-0) incompatible with serialization, [357](#page-377-0) finalizer attacks, and prevention, [30](#page-50-0)[–31](#page-51-0) finalizers, [29](#page-49-0)[–33](#page-53-0) alternative to, [31](#page-51-0) float for binary floating-point arithmetic, [270](#page-290-0) when to avoid, [270](#page-290-0)[–272](#page-292-0) fluent APIs, [14](#page-34-0), [203](#page-223-0) Flyweight pattern, [6](#page-26-0) footprint *See* space consumption for loops dual variable idiom, [263](#page-283-0) prefer for-each loops to, [264](#page-284-0)[–266](#page-286-0) vs. while loops, [262](#page-282-0) for-each loops limitations of, [266](#page-286-0) prefer over for loops, [264](#page-284-0)[–266](#page-286-0)

extralinguistic mechanisms cloning, [58](#page-78-0), [65](#page-85-0) native methods, [285](#page-305-0) reflection, [282](#page-302-0) serialization, [344](#page-364-0), [363](#page-383-0)

fork-join tasks and pools, [324](#page-344-0) formal type parameters, [117](#page-137-0) forwarding methods, [89](#page-109-0), [102](#page-122-0) frameworks callback, [91](#page-111-0) class-based, [281](#page-301-0) executor, [323](#page-343-0) interface-based, [6](#page-26-0) nonhierarchical type, [99](#page-119-0) service provider, [8](#page-28-0) function objects, [114](#page-134-0) vs. code blocks, [207](#page-227-0) functional interfaces, [193](#page-213-0) method overloading and, [243](#page-263-0) organization of standard, [200](#page-220-0)[–201](#page-221-0) using standard, [199](#page-219-0)[–202](#page-222-0) functional programming, [82](#page-102-0)

#### **G**

gadgets, [340](#page-360-0) garbage collection, [27,](#page-47-0) [29](#page-49-0)–[30,](#page-50-0) [113](#page-133-0) general contracts *See* contracts generic array creation errors, [126](#page-146-0)[–127,](#page-147-0) [133](#page-153-0) generic classes and interfaces, [117](#page-137-0) generic methods, [135](#page-155-0)[–138](#page-158-0) vs. unbounded wildcard types, [121](#page-141-0) generic singleton factories, [18](#page-38-0), [136](#page-156-0) generic type parameters *See* type parameters generic types, [14](#page-34-0), [117](#page-137-0), [130](#page-150-0)[–134](#page-154-0) documenting, [258](#page-278-0) immutability and, [136](#page-156-0) generic varargs parameter arrays heap pollution from, [147](#page-167-0)[–148](#page-168-0) replacing with lists, [149](#page-169-0) unsafe as storage, [146](#page-166-0) unsafe to expose, [147](#page-167-0)[–148](#page-168-0) generics, [117](#page-137-0)[–155](#page-175-0) boxed primitives and, [134](#page-154-0) compiler-generated casts and, [117](#page-137-0) erasure and, [126](#page-146-0) implementing atop arrays, [131](#page-151-0)[–133](#page-153-0) incompatibility with primitive types, [134](#page-154-0) invariant typing, [126](#page-146-0) varargs and, [127](#page-147-0), [146](#page-166-0)[–150](#page-170-0)

generifying existing code, [130](#page-150-0) Get and Put Principle, [141](#page-161-0)

#### **H**

hashCode method equals and, [48](#page-68-0), [50](#page-70-0)[–54](#page-74-0) general contract for, [50](#page-70-0) how to write, [51](#page-71-0) immutable objects and, [53](#page-73-0) heap pollution, [133](#page-153-0), [146](#page-166-0)[–148](#page-168-0) heap profilers, [28](#page-48-0) helper classes, [112](#page-132-0) for shortening parameter lists, [237](#page-257-0) hidden constructors, [61](#page-81-0), [96](#page-116-0), [339](#page-359-0), [344](#page-364-0), [353](#page-373-0) *See also* extralinguistic mechanisms hierarchical builder pattern, [16](#page-36-0)

#### **I**

immutable objects canonical forms and, [47](#page-67-0) clone and, [59](#page-79-0) dependency injection and, [21](#page-41-0) empty arrays and, [248](#page-268-0) enum types and, [160](#page-180-0) EnumSets and, [170](#page-190-0) failure atomicity and, [308](#page-328-0) functional approach and, [82](#page-102-0) hashCode and, [53](#page-73-0) JavaBeans and, [12](#page-32-0) mutable companion classes for, [84](#page-104-0) object reuse and, [22](#page-42-0) rules for, [80](#page-100-0) serialization and, [85](#page-105-0), [353](#page-373-0)[–358](#page-378-0) static factory methods and, [84](#page-104-0) subclassing and, [97](#page-117-0) thread safety and, [82](#page-102-0) imperative programming, [82](#page-102-0) implementation details documenting for inheritance, [94](#page-114-0) exposing, [92](#page-112-0) implementation inheritance, [87](#page-107-0) implementing interfaces, [4](#page-24-0) default methods and, [105](#page-125-0) inconsistent with equals, [67](#page-87-0)[–68](#page-88-0) unreliable resources and, [45](#page-65-0) information hiding *See* encapsulation

inheritance, [3](#page-23-0) vs. composition, [87](#page-107-0)[–92](#page-112-0) constructors and, [95](#page-115-0) designing for, [93](#page-113-0)[–98](#page-118-0) documenting for, [93](#page-113-0)[–94](#page-114-0) encapsulation and, [87](#page-107-0) fragility of, [89](#page-109-0) hooks to facilitate, [94](#page-114-0) implementation vs. interface, [3,](#page-23-0) [87](#page-107-0) of method doc comments, [259](#page-279-0) multiple, simulated, [102](#page-122-0) self-use of overridable methods and, [98](#page-118-0) uses of, [92](#page-112-0) *See also* subclassing initialization circularities, [333](#page-353-0), [366](#page-386-0) defensive copying and, [80](#page-100-0) of fields on deserialization, [351](#page-371-0) incomplete, [96](#page-116-0) lazy (*see* lazy initialization) of local variables, [261](#page-281-0) normal vs. lazy, [333](#page-353-0) at object creation, [86](#page-106-0) inner classes, [112](#page-132-0) Serializable and, [345](#page-365-0) to extend skeletal implementations, [102](#page-122-0) instance fields access levels of, [75](#page-95-0) initializing, [333](#page-353-0) lazy initialization of, [334](#page-354-0) vs. ordinals, [168](#page-188-0) instance-controlled classes, [6](#page-26-0), [158](#page-178-0) singletons, [17](#page-37-0)[–18](#page-38-0) static factory methods and, [6](#page-26-0) utility classes, [19](#page-39-0) *See also* enum types instanceof operator, parameter types, [121](#page-141-0) int constants vs. enum types, [157](#page-177-0)[–167](#page-187-0) int, for monetary calculations, [270](#page-290-0) interface-based frameworks, [6](#page-26-0), [99](#page-119-0)[–103](#page-123-0) interface inheritance, [87](#page-107-0) interfaces, [73](#page-93-0)[–114](#page-134-0) vs. abstract classes, [99](#page-119-0)[–103](#page-123-0) access levels of, [74](#page-94-0) accessibility of static members, [7](#page-27-0) default methods on, [99](#page-119-0), [104](#page-124-0)[–105](#page-125-0) for defining types, [107](#page-127-0)[–108](#page-128-0), [191](#page-211-0)[–192](#page-212-0) design of, [104](#page-124-0)[–106](#page-126-0) emulating extensible enums with, [176](#page-196-0)[–179](#page-199-0) enabling functionality enhancements, [100](#page-120-0) generic, [117](#page-137-0) marker (*see* marker interfaces) mixin, [58](#page-78-0), [99](#page-119-0) naming conventions for, [289](#page-309-0)[–291](#page-311-0) for nonhierarchical type frameworks, [99](#page-119-0) noninstantiable companion classes and, [7](#page-27-0) as parameter types, [170](#page-190-0), [237](#page-257-0) prefer to reflection, [282](#page-302-0) purpose of, [58](#page-78-0), [107](#page-127-0)[–108](#page-128-0) for referring to objects, [280](#page-300-0)[–281](#page-301-0) reflective instantiation of, [283](#page-303-0)–[284](#page-304-0) serialization and, [344](#page-364-0) skeletal implementations and, [100](#page-120-0)[–103](#page-123-0) static methods and, [7](#page-27-0) summary descriptions of, [257](#page-277-0) *See also individual interface names* internal field theft attacks, [360](#page-380-0)[–362](#page-382-0) invariant types, [126](#page-146-0), [139](#page-159-0) invariants clone and, [61](#page-81-0) concurrency and, [328](#page-348-0) constructors and, [82](#page-102-0), [86](#page-106-0) corruption of, [92](#page-112-0) enum types and, [362](#page-382-0) maintaining, [229](#page-249-0), [234](#page-254-0), [308](#page-328-0) of objects and members, [75](#page-95-0), [78](#page-98-0) **J** JavaBeans immutability and, [12](#page-32-0) method-naming conventions, [291](#page-311-0) pattern, [11](#page-31-0)[–12](#page-32-0) Javadoc, [254](#page-274-0) architecture documents and, [260](#page-280-0) class-level comments, [228](#page-248-0), [331](#page-351-0) client-side indexes in, [258](#page-278-0) comment inheritance, [259](#page-279-0) formatting, [255](#page-275-0)[–256](#page-276-0) module- and package-level comments, [259](#page-279-0) summary descriptions, [257](#page-277-0) **K**

key extractor functions, [70](#page-90-0)

**L** lambdas, [70](#page-90-0), [193](#page-213-0)[–225](#page-245-0) cleaners and, [33](#page-53-0) for constant-specific behaviors, [195](#page-215-0) prefer method references to, [197](#page-217-0)[–198](#page-218-0) prefer to anonymous classes, [193](#page-213-0)[–196](#page-216-0) serialization and, [196](#page-216-0) lazy initialization, [23](#page-43-0), [53](#page-73-0), [85](#page-105-0), [333](#page-353-0)[–335](#page-355-0) lazy initialization holder class idiom, [334](#page-354-0)[–335](#page-355-0) lazy initialization with a synchronized accessor idiom, [333](#page-353-0)[–334](#page-354-0) leaky abstractions, [146](#page-166-0) libraries, [267](#page-287-0)[–269](#page-289-0) Liskov substitution principle, [43](#page-63-0), [75](#page-95-0) listeners, avoiding memory leaks from, [28](#page-48-0) lists vs. arrays, [126](#page-146-0)[–129](#page-149-0) for generic varargs parameter arrays, [149](#page-169-0) mutual comparability in, [138](#page-158-0) liveness ensuring, [317](#page-337-0)[–322](#page-342-0), [328](#page-348-0) failures of, [224,](#page-244-0) [313](#page-333-0) local classes, [112](#page-132-0), [114](#page-134-0) local variables minimizing scope of, [261](#page-281-0)[–263](#page-283-0) naming conventions for, [290,](#page-310-0) [292](#page-312-0) locality of reference, [223](#page-243-0) locks fields containing, [332](#page-352-0) finalizers or cleaners and, [30](#page-50-0) private, [332](#page-352-0) reentrant, [320](#page-340-0) logical equality, [37](#page-57-0)[–38](#page-58-0) long, for monetary calculations, [270](#page-290-0) loops nested, [264](#page-284-0)[–266](#page-286-0) *See also* for loops; for-each loops **M** maps member classes and, [113](#page-133-0) nested, [173](#page-193-0)[–175](#page-195-0) vs. streams, behavior of, [173](#page-193-0) marker annotations, [181](#page-201-0) vs. marker interfaces, [191](#page-211-0) marker interfaces, [191](#page-211-0)[–192](#page-212-0) member classes, [112](#page-132-0)[–114](#page-134-0) *See also* static member classes members, [3](#page-23-0) minimizing accessibility of, [73](#page-93-0)[–77](#page-97-0) memory footprint *See* space consumption memory leaks, [26](#page-46-0)[–27](#page-47-0) nonstatic member classes and, [113](#page-133-0) self-management of memory, [28](#page-48-0) memory model, [80](#page-100-0) merge functions, [212](#page-232-0) meta-annotations, [181](#page-201-0) method chaining, [14](#page-34-0) method overloading, [238](#page-258-0)[–244](#page-264-0) accidental, of equals, [49](#page-69-0) effects of autoboxing and generics, [241](#page-261-0)[–242](#page-262-0) functional interfaces and, [202](#page-222-0), [243](#page-263-0) parameters and, [240](#page-260-0) static selection among methods, [238](#page-258-0) method overriding, [49](#page-69-0), [238](#page-258-0)[–239](#page-259-0) access levels and, [75](#page-95-0) clone, [58](#page-78-0) dynamic selection among methods, [238](#page-258-0) equals, [37](#page-57-0)[–49](#page-69-0) hashCode, [50](#page-70-0)[–54](#page-74-0) self-use and, [98](#page-118-0) toString, [55](#page-75-0)[–57](#page-77-0) unintentional, [190](#page-210-0) method references, [18](#page-38-0) kinds of, [198](#page-218-0) prefer to lambdas, [197](#page-217-0)[–198](#page-218-0) methods, [3](#page-23-0), [227](#page-247-0)[–260](#page-280-0) access levels of, [74](#page-94-0) accessor (*see* accessor methods) alien, [317](#page-337-0) common to all objects, [37](#page-57-0)[–72](#page-92-0) constant-specific, for enum-types, [162](#page-182-0) documenting, [254](#page-274-0)[–255](#page-275-0) exceptions thrown by, [304](#page-324-0)[–305](#page-325-0) overridable, [93](#page-113-0) summary descriptions of, [257](#page-277-0) thread safety of, [330](#page-350-0)[–332](#page-352-0) failure atomicity and, [308](#page-328-0)[–309](#page-329-0) forwarding (*see* forwarding methods) generic, [121](#page-141-0), [135](#page-155-0)[–138](#page-158-0) invocation, reflection and, [282](#page-302-0) legal for SafeVarargs, [149](#page-169-0)

methods (*continued*) minimizing accessibility of, [73](#page-93-0) naming conventions for, [9](#page-29-0), [290](#page-310-0)[–291](#page-311-0) native, [31](#page-51-0), [285](#page-305-0) nonfinal, and clone, [64](#page-84-0) overloading (*see* method overloading) overriding (*see* method overriding) parameter lists for, [236](#page-256-0) private, to capture wildcard types, [145](#page-165-0) shortening parameter lists of, [236](#page-256-0) signatures of, [3](#page-23-0), [236](#page-256-0)[–237](#page-257-0) size of, [263](#page-283-0) state-testing, vs. special return value, [295](#page-315-0) static factory (*see* static factory methods) SuppressWarnings annotation and, [124](#page-144-0) validity checking parameters, [227](#page-247-0)[–230](#page-250-0) varargs, [245](#page-265-0)–[246](#page-266-0) *See also individual method names* mixin interfaces, [58,](#page-78-0) [99](#page-119-0) mixing primitives, boxed primitives, [24](#page-44-0), [274](#page-294-0) modules, [76](#page-96-0)[–77](#page-97-0) monetary calculations, types for, [270](#page-290-0)[–271](#page-291-0) Monty Python reference, subtle, [247](#page-267-0) multi-catch facility, [320](#page-340-0) multiple inheritance, simulated, [102](#page-122-0) mutability JavaBeans pattern and, [11](#page-31-0) minimizing, [80](#page-100-0)[–86](#page-106-0) mutable companion classes, [84](#page-104-0) mutable reductions, [223](#page-243-0) mutators, [78](#page-98-0) mutual comparability, [138](#page-158-0) mutual exclusion, [311](#page-331-0) **N** named optional parameters, [14](#page-34-0) naming conventions, [236](#page-256-0), [289](#page-309-0)[–292](#page-312-0) of generic type parameters, [131](#page-151-0) grammatical, [291](#page-311-0)[–292](#page-312-0) of skeletal implementation classes, [101](#page-121-0) of static factory methods, [9](#page-29-0) streams and, [208](#page-228-0) of type parameters, [135](#page-155-0) naming patterns vs. annotations, [180](#page-200-0)[–187](#page-207-0) native methods, [31](#page-51-0), [285](#page-305-0) native peers, [31](#page-51-0) nested classes, [112](#page-132-0) access levels of, [74](#page-94-0) decreasing accessibility with, [74](#page-94-0) in serialization proxy pattern, [363](#page-383-0) types of, [112](#page-132-0) nested interfaces, access levels of, [74](#page-94-0) nested maps, [173](#page-193-0)[–175](#page-195-0) nonhierarchical type frameworks, [99](#page-119-0) noninstantiable classes, [19](#page-39-0) noninstantiable companion classes, [7](#page-27-0) non-nullity of equals, [38](#page-58-0), [45](#page-65-0) non-reifiable types, [127](#page-147-0), [131,](#page-151-0) [146](#page-166-0) nonstatic member classes, [112](#page-132-0)[–114](#page-134-0) notify vs. notifyAll, [328](#page-348-0)[–329](#page-349-0) null checking, [228](#page-248-0) nulling out obsolete object references, [27](#page-47-0) and, [46](#page-66-0) **O** object pools, [24](#page-44-0) object reference fields, equals and, [47](#page-67-0) objects, [3](#page-23-0) avoiding reflective access, [282](#page-302-0)[–284](#page-304-0) base classes and, [281](#page-301-0) creating and destroying, [5](#page-25-0)[–33](#page-53-0) creation and performance, [6](#page-26-0), [22](#page-42-0)[–23](#page-43-0) deserialization filtering of, [342](#page-362-0) effectively immutable, [316](#page-336-0) eliminating obsolete references to, [26](#page-46-0)[–28](#page-48-0), [60](#page-80-0), [308](#page-328-0) expense of creating, [24](#page-44-0) favor refering to by interfaces, [280](#page-300-0)[–281](#page-301-0) function, [114](#page-134-0) immutable (*see* immutable objects) in inconsistent states, [11](#page-31-0)[–12](#page-32-0), [96](#page-116-0), [309](#page-329-0) (*see also* corrupted objects) methods common to all, [37](#page-57-0)[–72](#page-92-0) nulling out obsolete references to, [27](#page-47-0) process, [114](#page-134-0) reuse, [22](#page-42-0)[–25](#page-45-0) safe publication of, [316](#page-336-0) string representations of, [55](#page-75-0)[–57](#page-77-0) when to refer to by class, [281](#page-301-0) Observer pattern, [317](#page-337-0) obsolete object references, [26](#page-46-0)[–28](#page-48-0), [60](#page-80-0), [308](#page-328-0) open calls, [321](#page-341-0)

natural ordering, [66](#page-86-0)

NullPointerException, equals contract

optimizations, [286](#page-306-0)[–288](#page-308-0) caching hash codes, [53](#page-73-0) lazy initialization, [333](#page-353-0)[–335](#page-355-0) notify instead of notifyAll, [329](#page-349-0) object reuse, [22](#page-42-0)[–25](#page-45-0) order of comparisons in equals, [47](#page-67-0) parallelizing streams, [224](#page-244-0) static initialization, [23](#page-43-0) StringBuffer and, [279](#page-299-0) using == in equals, [46](#page-66-0) optionals, [249](#page-269-0) exceptions and, [295](#page-315-0), [298](#page-318-0) as return values, [249](#page-269-0)[–253](#page-273-0) ordinals vs. enum maps, [171](#page-191-0)[–175](#page-195-0) vs. instance fields, [168](#page-188-0) overloading *See* method overloading Override annotations, [49](#page-69-0), [188](#page-208-0)[–190](#page-210-0) overriding *See* method overriding **P** package-private access level, [4,](#page-24-0) [74](#page-94-0), [84](#page-104-0) packages, naming conventions for, [289](#page-309-0)[–290](#page-310-0) parallelizing streams, [222](#page-242-0)[–225](#page-245-0) parameter lists of builders, [14](#page-34-0) shortening, [236](#page-256-0)[–237](#page-257-0) varargs and, [245](#page-265-0)[–246](#page-266-0) parameterized types, [117](#page-137-0)[–122](#page-142-0) reifiable, [127](#page-147-0) parameterless constructors, [19](#page-39-0) parameters defensive copies of mutable, [232](#page-252-0) type (*see* type parameters) validity checking of, [227](#page-247-0)[–230](#page-250-0), [353](#page-373-0)[–355](#page-375-0) PECS mnemonic, [141](#page-161-0) performance, [286](#page-306-0)[–288](#page-308-0) autoboxing and, [24](#page-44-0), [201](#page-221-0), [275](#page-295-0) BigDecimal and, [271](#page-291-0) builder pattern, [16](#page-36-0) cleaners, [29](#page-49-0)[–30](#page-50-0) defensive copying and, [234](#page-254-0) of enums, [167](#page-187-0), [170](#page-190-0) of equals, [46](#page-66-0)[–47](#page-67-0) of excessive synchronization, [321](#page-341-0) finalizers, [29](#page-49-0)[–30](#page-50-0) for-each loops and, [264](#page-284-0) of hashCode, [50](#page-70-0), [53](#page-73-0) immutable classes and, [83](#page-103-0)[–85](#page-105-0) libraries and, [268](#page-288-0) measuring, [287](#page-307-0) memory leaks and, [27](#page-47-0) native methods and, [285](#page-305-0) object creation and, [6](#page-26-0), [22](#page-42-0)[–23](#page-43-0) of reflection, [282](#page-302-0) parallelizing streams and, [222](#page-242-0)[–225](#page-245-0) of serialization, [348](#page-368-0)[–350](#page-370-0) software architecture and, [286](#page-306-0)[–287](#page-307-0) state-testing vs. special return value, [295](#page-315-0) static factories and, [6](#page-26-0) of string concatenation, [279](#page-299-0) toString and, [57](#page-77-0) varargs and, [246](#page-266-0) wrapper classes and, [91](#page-111-0) *See also* optimizations performance model, [288](#page-308-0) portability cleaners and, [29](#page-49-0) finalizers and, [29](#page-49-0) native methods and, [285](#page-305-0) thread priorities and, [337](#page-357-0) thread scheduler and, [336](#page-356-0) predicates, [104](#page-124-0) primitive fields compareTo and, [69](#page-89-0) equals and, [47](#page-67-0) primitive types, [273](#page-293-0) incompatibility with generic types, [134](#page-154-0) optionals and, [253](#page-273-0) prefer over boxed primitives, [24](#page-44-0), [273](#page-293-0)[–275](#page-295-0) *See also individual primitive types* private access level, [74](#page-94-0) private constructors, [84](#page-104-0) for noninstantiability, [19](#page-39-0) for singletons, [17](#page-37-0)[–18](#page-38-0) private lock object idiom, [332](#page-352-0) private lock objects, [332](#page-352-0) procedural programming, [82](#page-102-0) process objects, [114](#page-134-0) producer-consumer queues, [326](#page-346-0) programming principles, [2](#page-22-0) promptness of finalization, [29](#page-49-0)

protected access level, [74](#page-94-0)[–75](#page-95-0) public access level, [74](#page-94-0) public fields vs. accessor methods, [78](#page-98-0)[–79](#page-99-0) publicly accessible locks, [331](#page-351-0) **Q** qualified this construct, [112](#page-132-0) **R** racy single check idiom, [335](#page-355-0) range checking, [229](#page-249-0) raw types, [117](#page-137-0)[–122](#page-142-0) readObject method, [353](#page-373-0)[–358](#page-378-0) defensive copies and, [80](#page-100-0) how to write, [358](#page-378-0) incompatible with instance-controlled objects, [359](#page-379-0) overridable methods and, [96](#page-116-0), [358](#page-378-0) readResolve method access levels of, [97](#page-117-0) choosing access levels of, [362](#page-382-0) prefer enum types to, [359](#page-379-0)[–362](#page-382-0) using for instance-controlled classes, [359](#page-379-0) recipes adding behaviors to individual enum constants, [162](#page-182-0) adding data to enum types, [160](#page-180-0) builder pattern, [12](#page-32-0) checking significant fields in equals, [47](#page-67-0) clone, [64](#page-84-0) compareTo, [68](#page-88-0) eliminating self-use, [98](#page-118-0) equals, [46](#page-66-0) generifying a class, [130](#page-150-0)[–133](#page-153-0) hashCode, [51](#page-71-0) implementing generics atop arrays, [131](#page-151-0)[–133](#page-153-0) method chaining, [14](#page-34-0) noninstantiable classes, [19](#page-39-0) readObject, [358](#page-378-0) serialization proxies, [363](#page-383-0)[–364](#page-384-0) serialized singletons, [18](#page-38-0) singletons as single-element enums, [18](#page-38-0) singletons with private constructors, [17](#page-37-0) skeletal implementations, [102](#page-122-0) tagged classes to class hierarchies, [110](#page-130-0)[–111](#page-131-0) *See also* rules

recursive type bounds, [137](#page-157-0) recursive type parameters, [14](#page-34-0) reduction strategy, [211](#page-231-0) reductions, [223](#page-243-0) reentrant locks, [320](#page-340-0) reference types, [3](#page-23-0), [273](#page-293-0) reflection, [282](#page-302-0)[–284](#page-304-0) AccessibleObject.setAccessible attacks, [17](#page-37-0) clone and, [58](#page-78-0) drawbacks of, [282](#page-302-0) reflective interface instantiation, [283](#page-303-0) uses for, [282](#page-302-0), [284](#page-304-0) reflexivity requirements compareTo, [68](#page-88-0) equals, [38](#page-58-0)[–39](#page-59-0) reified types, [126](#page-146-0) resource factories, [21](#page-41-0) resource-ordering deadlocks, [351](#page-371-0) resources locked, and finalizers, [30](#page-50-0) releasing, [31](#page-51-0) restricted marker interfaces, [191](#page-211-0) return classes, varied serialization proxy pattern and, [365](#page-385-0) static factory methods and, [7](#page-27-0)[–8](#page-28-0) return statements, SuppressWarnings annotation and, [124](#page-144-0) return types bounded wildcard types as, [142](#page-162-0) collections vs. streams, [216](#page-236-0)[–221](#page-241-0) static factory methods and, [6](#page-26-0) reusable forwarding classes, [89](#page-109-0)[–91](#page-111-0) rules accessibility, [74](#page-94-0)[–75](#page-95-0) appropriateness of checked exceptions, [298](#page-318-0) choosing bounded wildcard types, [141](#page-161-0) choosing exception types, [296](#page-316-0)[–297](#page-317-0) decreasing serialization dangers, [341](#page-361-0)[–342](#page-362-0) for immutable objects, [80](#page-100-0) mapping domains to package names, [289](#page-309-0) marker interfaces vs. annotations, [192](#page-212-0) optimization, [286](#page-306-0) for performance of parallel streams, [223](#page-243-0) replacing type parameters with wildcards, [144](#page-164-0)

for SafeVarargs annotations, [149](#page-169-0) static members accessibility, [112](#page-132-0) writing doc comments, [254](#page-274-0)[–260](#page-280-0) runtime exceptions *See* unchecked exceptions **S** safe array accesses, [76](#page-96-0) safe languages, [231](#page-251-0) safe publication, [316](#page-336-0) safety failures, [315](#page-335-0) parallel streams and, [224](#page-244-0) wait and, [328](#page-348-0) SafeVarargs annotations, [147](#page-167-0) legal uses of, [149](#page-169-0) scope local variables, [261](#page-281-0)[–263](#page-283-0) SuppressWarnings annotations, [124](#page-144-0) of variables, obsolete references and, [27](#page-47-0) security, defensive copying for, [25](#page-45-0), [231](#page-251-0) security issues accessible nonzero-length arrays, [76](#page-96-0) AccessibleObject.setAccessible attacks, [17](#page-37-0) denial of service attacks, [331](#page-351-0) deserialization bombs, [339](#page-359-0)[–340](#page-360-0) ElvisStealer attacks, [362](#page-382-0) finalizer attacks, [30](#page-50-0)[–31](#page-51-0) gadgets, [340](#page-360-0) internal field theft attacks, [355](#page-375-0)[–357](#page-377-0) ransomware attacks, [339](#page-359-0) reflection, [17](#page-37-0) remote code execution, [340](#page-360-0) rogue object reference attacks, [355](#page-375-0)[–357](#page-377-0) serialization, [339](#page-359-0), [344](#page-364-0), [353](#page-373-0), [360](#page-380-0) stealer attacks, [362](#page-382-0) strings as keys for granting data access, [277](#page-297-0) subclassing and, [89](#page-109-0) time-of-check/time-of-use (TOCTOU) attacks, [233](#page-253-0) SELF problem, [91](#page-111-0) self-use documenting, for inheritance, [93](#page-113-0) eliminating, for inheritance, [98](#page-118-0) serial version UIDs, [343](#page-363-0), [351](#page-371-0)[–352](#page-372-0) Serializable, [343](#page-363-0)[–345](#page-365-0) serialization, [339](#page-359-0)[–366](#page-386-0) anonymous classes and, [196](#page-216-0) costs of, [343](#page-363-0) decreasing the dangers of, [341](#page-361-0)[–342](#page-362-0) designing for inheritance and, [96](#page-116-0)[–97](#page-117-0) documenting for, [347](#page-367-0), [350](#page-370-0) effect on exported APIs, [343](#page-363-0) flexible return classes for, [365](#page-385-0) immutability and, [85](#page-105-0), [353](#page-373-0) internal field theft attacks and, [360](#page-380-0)[–362](#page-382-0) lambdas and, [196](#page-216-0) object deserialization filtering, [342](#page-362-0) prefer alternatives to, [339](#page-359-0)[–342](#page-362-0) singletons and, [18](#page-38-0) synchronization for, [351](#page-371-0) transient fields for, [348](#page-368-0) validity checking in, [357](#page-377-0) when to use, [345](#page-365-0) serialization proxy pattern, [363](#page-383-0)[–366](#page-386-0) serialized forms, as part of exported APIs, [343](#page-363-0) serialized instances vs. serialization proxy pattern, [363](#page-383-0)[–366](#page-386-0) service provider frameworks, [8](#page-28-0) short-circuiting operations, [223](#page-243-0) signatures of methods, [3](#page-23-0), [236](#page-256-0)[–237](#page-257-0) signum function, [67](#page-87-0) simple implementations, [103](#page-123-0) simulated multiple inheritance, [102](#page-122-0) simulated self-type idiom, [14](#page-34-0) single-check idiom, [335](#page-355-0) singletons, [17](#page-37-0)[–18](#page-38-0) vs. dependency injection, [20](#page-40-0) skeletal implementations, [100](#page-120-0)[–101](#page-121-0) source files, [115](#page-135-0)[–116](#page-136-0) space consumption enum types, [175](#page-195-0) immutable objects and, [83](#page-103-0) memory leaks and, [27](#page-47-0) nonstatic member classes and, [113](#page-133-0) spliterator, [223](#page-243-0) spurious wake-ups, [329](#page-349-0) state-dependent modify operations, [325](#page-345-0) state-testing methods, [294](#page-314-0)[–295](#page-315-0), [299](#page-319-0)

static factory methods, [5](#page-25-0) advantages of, [5](#page-25-0)[–8](#page-28-0) anonymous classes within, [114](#page-134-0) in API documentation, [8](#page-28-0) vs. cloning, [65](#page-85-0) copy and conversion factories, [65](#page-85-0) flexibility in returned classes, [7](#page-27-0)[–8](#page-28-0) for generic singletons, [18](#page-38-0), [136](#page-156-0) immutable objects and, [22](#page-42-0), [82](#page-102-0), [84](#page-104-0) instance-controlled classes and, [6](#page-26-0) limitations of, [8](#page-28-0)[–9](#page-29-0) naming conventions for, [9](#page-29-0), [292](#page-312-0) replacing constructors with, [5](#page-25-0)[–9](#page-29-0), [22](#page-42-0), [240](#page-260-0) return types of, [6](#page-26-0)[–8](#page-28-0) for service provider frameworks, [8](#page-28-0) for singletons, [17](#page-37-0) subclassing and, [8](#page-28-0) static fields for defining constants, [290](#page-310-0) lazy initialization of, [334](#page-354-0) synchronization of mutable, [322](#page-342-0) static import facility, [108](#page-128-0) static imports, [70](#page-90-0) static member classes, [112](#page-132-0) cleaners and, [33](#page-53-0) common uses of, [112](#page-132-0)[–113](#page-133-0) for enum types, [161](#page-181-0) vs. nonstatic, [112](#page-132-0), [114](#page-134-0) for representing aggregates, [276](#page-296-0) for shortening parameter lists, [237](#page-257-0) static members, accessibility in interfaces, [7](#page-27-0) storage pools, [28](#page-48-0) strategy enum pattern, [166](#page-186-0) Strategy pattern, [193](#page-213-0) stream pipelines, [203](#page-223-0) side-effect free, [210](#page-230-0)[–215](#page-235-0) stream unique identifiers *See* serial version UIDs streams, [193](#page-213-0), [203](#page-223-0)[–225](#page-245-0) char values and, [206](#page-226-0) collectors for, [211](#page-231-0)[–215](#page-235-0) for functional programming, [210](#page-230-0)[–215](#page-235-0) vs. maps, behavior of, [173](#page-193-0) parallelizing, [222](#page-242-0)[–225](#page-245-0) preserving order from parallel, [224](#page-244-0) as return types, vs. collections, [216](#page-236-0)[–221](#page-241-0) specifying collectors for, [173](#page-193-0), [214](#page-234-0) strengths of, [207](#page-227-0) vs. threads, [323](#page-343-0)[–324](#page-344-0) using, [203](#page-223-0)[–209](#page-229-0) *See also* collectors String constants vs. enum types, [158](#page-178-0) string representations, [55](#page-75-0)[–57](#page-77-0), [306](#page-326-0) strings concatenating, [279](#page-299-0) as substitutes for other types, [276](#page-296-0)[–278](#page-298-0) subclassing, [3](#page-23-0), [87](#page-107-0) abstract classes, and equals, [45](#page-65-0) access levels and, [75](#page-95-0) appropriateness of, [92](#page-112-0) Cloneable and, [96](#page-116-0) compareTo and, [68](#page-88-0) equals and, [40](#page-60-0), [42](#page-62-0) finalizer attacks and, [31](#page-51-0) fragility, [89](#page-109-0) invariant corruption and, [92](#page-112-0) method access levels and, [75](#page-95-0) prohibiting, [8](#page-28-0), [18](#page-38-0)[–19](#page-39-0), [85](#page-105-0), [97](#page-117-0) serialization and, [344](#page-364-0) skeletal implementations, [102](#page-122-0) static factory methods and, [8](#page-28-0) as test of design for inheritance, [95](#page-115-0) *See also* inheritance subtype relations, [134](#page-154-0), [140](#page-160-0) summary descriptions in Javadoc, [257](#page-277-0) supertype relations, [141](#page-161-0) SuppressWarnings annotation, [123](#page-143-0)[–125](#page-145-0) switch statements, and enum types, [164](#page-184-0), [167](#page-187-0) symmetry requirements compareTo, [68](#page-88-0) equals, [38](#page-58-0)[–39](#page-59-0) synchronization of atomic data, [312](#page-332-0)[–314](#page-334-0) excessive, [317](#page-337-0)[–322](#page-342-0) and performance, [321](#page-341-0) ramifications of, [317](#page-337-0) internal, [322](#page-342-0) for mutual exclusion, [311](#page-331-0) serialization and, [351](#page-371-0) for shared mutable data, [311](#page-331-0)[–316](#page-336-0) techniques for, [314](#page-334-0)[–316](#page-336-0) for thread communication, [312](#page-332-0)[–314](#page-334-0)

synchronized regions alien methods and, [317](#page-337-0)[–321](#page-341-0) minimizing work in, [321](#page-341-0) synchronizers, [326](#page-346-0) synthetic annotations, [186](#page-206-0)

#### **T**

tag fields, [109](#page-129-0) tardy finalization, [29](#page-49-0) tasks, [324](#page-344-0) vs. threads, [323](#page-343-0)[–324](#page-344-0) telescoping constructor pattern, [10](#page-30-0)[–11](#page-31-0) Template Method pattern, [101](#page-121-0), [199](#page-219-0) this, in doc comments, [256](#page-276-0) this, in lambdas vs. anonymous classes, [196](#page-216-0) thread pools, [323](#page-343-0) sizing of, [336](#page-356-0) thread priorities, [337](#page-357-0) thread safety documenting, [322](#page-342-0), [330](#page-350-0)[–332](#page-352-0) immutable objects and, [82](#page-102-0) levels of, [330](#page-350-0)[–331](#page-351-0) mutability and, [75](#page-95-0), [322](#page-342-0) thread schedulers, [336](#page-356-0)[–337](#page-357-0) thread starvation deadlocks, [328](#page-348-0) Thread.yield method, avoiding, [337](#page-357-0) threads, busy-waiting, [336](#page-356-0) throwables, types of, [296](#page-316-0) time-of-check/time-of-use attacks, [233](#page-253-0) TOCTOU attacks, [233](#page-253-0) toString method, [55](#page-75-0)[–57](#page-77-0) enum types and, [160](#page-180-0) general contract for, [55](#page-75-0) when to override, [57](#page-77-0) transient fields, [348](#page-368-0)[–351](#page-371-0) with readResolve, [360](#page-380-0) when to use, [351](#page-371-0) transitivity requirements compareTo, [68](#page-88-0) equals, [38](#page-58-0), [40](#page-60-0)[–45](#page-65-0) try-finally prefer try-with-resources to, [34](#page-54-0) try-with-resources prefer to try-finally, [34](#page-54-0)[–36](#page-56-0) type bounds, recursive, [137](#page-157-0) type inference, [70](#page-90-0), [123](#page-143-0), [142,](#page-162-0) [194](#page-214-0)

type parameter lists, [135](#page-155-0) type parameters, [117](#page-137-0), [135](#page-155-0) bounded, [134](#page-154-0), [154](#page-174-0)

naming conventions for, [135](#page-155-0), [290](#page-310-0) recursively bound, [14](#page-34-0), [137](#page-157-0) vs. wildcards, [144](#page-164-0)

type safety

dynamic casts and, [154](#page-174-0) from enum types, [158](#page-178-0) heap pollution and, [146](#page-166-0) parameterized types and, [119](#page-139-0)

raw types and, [119](#page-139-0) type tokens, [151](#page-171-0)

types

conversion of, [65](#page-85-0), [291](#page-311-0) generic, [117](#page-137-0), [130](#page-150-0)[–134](#page-154-0)

interfaces for defining, [107](#page-127-0)[–108](#page-128-0), [191](#page-211-0)[–192](#page-212-0)

non-reifiable, [127](#page-147-0), [131](#page-151-0) parameterized, [117](#page-137-0)[–122](#page-142-0)

primitive, [273](#page-293-0) raw, [117](#page-137-0) reference, [273](#page-293-0)
...
[151](#page-171-0)[–155](#page-175-0)

incompatibility with nonreifiable types, [154](#page-174-0)

#### **U**

unbounded type parameters vs. bounded wildcard types, [144](#page-164-0) unbounded wildcard types, [120](#page-140-0) vs. bounded wildcard types, [121](#page-141-0) nested, [152](#page-172-0) vs. raw types, [121](#page-141-0) reifiable, [127](#page-147-0) vs. unbounded type parameters, [144](#page-164-0) unchecked exceptions vs. checked exceptions, [296](#page-316-0)[–297](#page-317-0) compatibility and, [305](#page-325-0) excluding from method declarations, [304](#page-324-0) purpose of, [296](#page-316-0) unchecked warnings, [123](#page-143-0)[–125](#page-145-0) of casts, [127](#page-147-0), [129](#page-149-0), [137](#page-157-0) underscores, in numeric literals, [108](#page-128-0) unintentional object retentions *See* memory leaks

unintentionally instantiable classes, [19](#page-39-0) users of APIs, [4](#page-24-0) utility classes, [19](#page-39-0) vs. constant interfaces, [108](#page-128-0) vs. dependency injection, [20](#page-40-0)

#### **V**

validity checking builders and, [14](#page-34-0) constructor parameters, [229](#page-249-0) defensive copying and, [232](#page-252-0) of deserialized objects, [355](#page-375-0) implicit, [230](#page-250-0) parameters, [227](#page-247-0)[–230](#page-250-0) failure atomicity and, [308](#page-328-0) readObject parameters, [353](#page-373-0)[–355](#page-375-0) value classes, [38](#page-58-0) toString and, [56](#page-76-0) value types vs. strings, [276](#page-296-0) varargs, [245](#page-265-0)–[246](#page-266-0) builders and, [16](#page-36-0) with generics, [146](#page-166-0)[–150](#page-170-0) generics, and compiler warnings, [127](#page-147-0) performance, [246](#page-266-0) variable arity methods, [245](#page-265-0) variable return classes serialization proxy pattern and, [365](#page-385-0) static factory methods and, [7](#page-27-0)[–8](#page-28-0) variables atomic operations on, [311](#page-331-0) local (*see* local variables)

naming conventions for, [290](#page-310-0) scope of, and obsolete references, [27](#page-47-0) to avoid subclassing, [44](#page-64-0), [68](#page-88-0) to maintain invariants, [234](#page-254-0) naming conventions for, [291](#page-311-0) object reuse and, [23](#page-43-0) volatile modifier, [314](#page-334-0)[–315](#page-335-0)

### **W**

wait loop idiom, [328](#page-348-0)[–329](#page-349-0) warnings, unchecked *See* unchecked warnings weak references, [28](#page-48-0) while loops vs. for loops, [262](#page-282-0) wildcard types capturing, [145](#page-165-0) vs. type parameters, [144](#page-164-0) *See also* bounded wildcard types; unbounded wildcard types window of vulnerability, [233](#page-253-0) work queues, [326](#page-346-0) wrapper class idiom, [100](#page-120-0) wrapper classes, [89](#page-109-0)[–91](#page-111-0) defensive copying and, [235](#page-255-0) incompatible with callback frameworks, [91](#page-111-0) vs. subclassing, [97](#page-117-0) writeReplace method, access levels of, [97](#page-117-0)

#### **Z**

zero-length arrays, immutability of, [248](#page-268-0)