...
import type { ApolloView, QuizSummary, NormalizedQuestion, QuizPreset } from "./types";
import { fetchQuizzes, fetchQuiz, ingestQuiz } from "./api";
import { normalizeQuiz, filterBySection, sampleQuiz } from "./normalizeQuiz";
...
export default function Apollo() {
  ...
  // Selected quiz — all questions (unsampled)
  const [selectedId, setSelectedId] = useState<string | null>(null);
  const [allQuestions, setAllQuestions] = useState<NormalizedQuestion[]>([]);
  const [quizTitle, setQuizTitle] = useState("");

  // Quiz config — the questions that will actually be used
  ...
  const applyPreset = useCallback((preset: QuizPreset) => {
    setTfCount(Math.min(preset.tf, available.tf));
    setMcCount(Math.min(preset.mc, available.mc));
    setSaCount(Math.min(preset.sa, available.sa));
  }, [available]);

  const applyHalf = useCallback(() => {
  ...
  const startQuiz = useCallback(() => {
    ...
    setView("quiz");
  }, [allQuestions, tfCount, mcCount, saCount]);
  ...
  const handleUpload = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        setUploadMsg(
          `Loaded ${result.total_questions} questions from ${result.quiz_ids.length} quiz(es)`,
        );
        await loadQuizzes();
      } catch (err) {
  ...
  const handleExit = useCallback(() => { setView("select"); }, []);
  ...
  if (view === "quiz-config" && allQuestions.length > 0) {
    return (
      <div className="quiz-config">
        <div className="study-header">
          <button className="study-exit" onClick={() => setView("select")}>
            &#8249; Back
          </button>
          <span className="study-counter">{quizTitle}</span>
        </div>
        ...
        <div className="quiz-config-body">
          ...
          <div className="quiz-config-presets">
            {PRESETS.map((p) => (
              <button
                key={p.id}
                className="quiz-config-preset"
                onClick={() => applyPreset(p)}
              >
                <span className="preset-label">{p.label}</span>
                <span className="preset-detail">
                  {p.tf} T/F, {p.mc} MC, {p.sa} SA
                </span>
              </button>
            ))}
